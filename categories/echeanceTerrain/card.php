...
require_once(DOL_DOCUMENT_ROOT .'/categories/class/echeanceTerrain.class.php');
...
{  /* echo '<pre>';
    ...
    $obj= $db->fetch_object($resql);
        $object = new echeanceTerrain($db);
        $object->periodicite = GETPOST('periodicite', 'int');
        ...
        $object->nb_ligne =$object->duree/$object->periodicite;
//
        $object->montant =price(round($obj->total/$object->nb_ligne)) ;
    //var_dump( $object->montant);die();
        $object->fk_propal = $idPropal;
...
//rowid de la nouvelle échéance
    ...
    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."echeanceTerrain as e ON e.fk_propal=p.rowid";
    $sql.=" WHERE p.rowid = ".$originid;
    $result = $db->query($sql);
    $objet = $db->fetch_object($result);
    $i=0;
    while ($i < $object->nb_ligne) {
        $fact = new facture($db);
...
//var_dump(1111);die();
            ...
            $ref='(ACC_'.$idf.')';
            $sqlu = 'UPDATE '.MAIN_DB_PREFIX."facture SET ref='".$db->escape($ref)."' WHERE rowid=".$idf;
         // var_dump($sqlu);die();
            $resqlu = $db->query($sqlu);
            //var_dump($db->query($sqlu));
           /* $sql = 'UPDATE '.MAIN_DB_PREFIX."echeanceTerrain SET fk_facture='".$db->escape($id)."' WHERE rowid=".$rowid;
            $resql= $db->query($sql);*/
        ...
        /*else  {
            ...
            header("location: /categories/echeanceTerrain/card.php?action=addEcheance&origin=propal&originid=$originid&socid=$socid");
            die;
        }*/


        $sqletd = "INSERT INTO " . MAIN_DB_PREFIX . "echeanceTerraindet(";

        $sqletd.= "fk_echeanceTerrain";
        $sqletd.= ", fk_facture";
        $sqletd.= ")";
        $sqletd .= "VALUES (";
        $sqletd .= "" . (!isset($rowid) ? 'NULL' : "'" . $db->escape($rowid) . "'") . ",";
        $sqletd .= "" . (!isset($idf) ? 'NULL' : "'" . $db->escape($idf) . "'"). ",";
...
elseif ($action == 'update') {
    ...
    if ($resql > 0)
    {
        ...
        header("location: /comm/propal/card.php?id=$originid");die();
            ...
            <form action="<?= $_SERVER["PHP_SELF"] ?>" method="POST">
                ...
                <div class="tabBar tabBarWithBottom">
                    <table class="border" width="100%">
                        ...
                        <tr>
                            <td class="titlefieldcreate fieldrequired"><?= $langs->trans('durée') ?> (mois)</td>
                            <td><input class="flat" type = "number" size="40" name="duree" min="0" required="required" ></td>
                        </tr>

                        <tr>
                            <td class="titlefieldcreate fieldrequired"><?= $langs->trans('périodicité') ?> (mois)</td>
                            <td><input class="flat" type = "number" size="40" name="periodicite" min="0" required="required" ></td>
                        </tr>
