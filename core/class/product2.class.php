 ...
 */
class Product2 extends CommonObject
{
	var $db;
	var $error;
	//! Error number
	...
	function getMultiLangs()
	{
		...
		if ($result)
		{
			...
			{
				//print 'lang='.$objp->lang.' current='.$current_lang.'<br>';
				if( $objp->lang == $current_lang ) // si on a les traduct. dans la langue courante on les charge en infos principales.
				{
					$this->libelle		= $objp->label;
					$this->description	= $objp->description;
					$this->note			= $objp->note;

				}
				$this->multilangs["$objp->lang"]["libelle"]		= $objp->label;
				$this->multilangs["$objp->lang"]["description"]	= $objp->description;
				$this->multilangs["$objp->lang"]["note"]			= $objp->note;
			}
		}
		else
		{
	...
	function get_buyprice($prodfournprice,$qty,$product_id=0,$fourn_ref=0)
	{
		...
		if ($resql)
		{
			...
			if ($objp && $objp->quantity > 0)
			{
				$this->buyprice = $objp->price;                      // \deprecated
				$this->fourn_pu = $objp->price / $objp->quantity;     // Prix unitaire du produit pour le fournisseur $fourn_id
				$this->ref_fourn = $objp->ref_fourn;
				$result=$objp->fk_product;
				return $result;
			}
			else
			{
	...
	{
	    //include_once(DOL_DOCUMENT_ROOT.'/compta/mesdocs/company.lib.php');
		...
		if ( $resql )
		{
			...
			{
				$objpect = $this->db->fetch_object($resql);

				$this->id					= $objpect->rowid;
				$this->ref					= $objpect->ref;
				$this->libelle				= $objpect->label;		// TODO deprecated
				$this->label				= $objpect->label;
				$this->description			= $objpect->description;
				$this->note					= $objpect->note;
	            $this->customcode			= $objpect->customcode;
	            $this->country_id			= $objpect->fk_country;
	            $this->country_code			= getCountry($this->country_id,2,$this->db);
	            $this->price				= $objpect->price;
				$this->price_ttc			= $objpect->price_ttc;
				$this->price_min			= $objpect->price_min;
				$this->price_min_ttc		= $objpect->price_min_ttc;
				$this->price_base_type		= $objpect->price_base_type;
				$this->tva_tx				= $objpect->tva_tx;
				//! French VAT NPR
				$this->tva_npr				= $objpect->tva_npr;
				//! Spanish local taxes
				$this->localtax1_tx			= $objpect->localtax1_tx;
				$this->localtax2_tx			= $objpect->localtax2_tx;

				$this->type					= $objpect->fk_product_type;
				$this->status				= $objpect->tosell;
				$this->status_buy			= $objpect->tobuy;
				$this->finished				= $objpect->finished;
				$this->hidden				= $objpect->hidden;
				$this->duration				= $objpect->duration;
				$this->duration_value		= substr($objpect->duration,0,dol_strlen($objpect->duration)-1);
				$this->duration_unit		= substr($objpect->duration,-1);
				$this->seuil_stock_alerte	= $objpect->seuil_stock_alerte;
				$this->canvas				= $objpect->canvas;
				$this->weight				= $objpect->weight;
				$this->weight_units			= $objpect->weight_units;
				$this->length				= $objpect->length;
				$this->length_units			= $objpect->length_units;
				$this->surface				= $objpect->surface;
				$this->surface_units		= $objpect->surface_units;
				$this->volume				= $objpect->volume;
				$this->volume_units			= $objpect->volume_units;
				$this->barcode				= $objpect->barcode;
				$this->barcode_type			= $objpect->fk_barcode_type;

				$this->accountancy_code_buy = $objpect->accountancy_code_buy;
				$this->accountancy_code_sell= $objpect->accountancy_code_sell;

				$this->stock_reel         = $objpect->stock;
				$this->pmp                = $objpect->pmp;

				$this->import_key         = $objpect->import_key;
	...
	function fetch2($id='',$ref='')
	{
		...
		//mysql_select_db($database_con_gescom, $con_gescom);
		// $rq_sql2 = mysql_query($sql2, $con_gescom);
		$db1 = new DoliDb($conf->db->type,$conf->db->host,$conf->db->user,$conf->db->pass,$conf->db->name,$conf->db->port);
		$this->db = $db1;
		$resql = $db1->query($sql);
		if ( $resql )
		{
			if ($db1->num_rows($resql) > 0)
			{
				$objpect = $db1->fetch_object($resql);
	...
	function fetch3($id='',$ref='',$nbniveau=0)
	{
	    include_once(DOL_DOCUMENT_ROOT.'/compta/mesdocs/company.lib.php');
	...
	function load_stats_propale($socid=0)
	{
		...
		{
			$objp=$this->db->fetch_object($result);
			$this->stats_propale['customers']=$objp->nb_customers;
			$this->stats_propale['nb']=$objp->nb;
			$this->stats_propale['rows']=$objp->nb_rows;
			$this->stats_propale['qty']=$objp->qty?$objp->qty:0;
			return 1;
		}
		else
		{
	...
	function load_stats_commande($socid=0,$filtrestatut='')
	{
		...
		{
			$objp=$this->db->fetch_object($result);
			$this->stats_commande['customers']=$objp->nb_customers;
			$this->stats_commande['nb']=$objp->nb;
			$this->stats_commande['rows']=$objp->nb_rows;
			$this->stats_commande['qty']=$objp->qty?$objp->qty:0;
			return 1;
		}
		else
		{
	...
	function load_stats_commande_fournisseur($socid=0,$filtrestatut='')
	{
		...
		{
			$objp=$this->db->fetch_object($result);
			$this->stats_commande_fournisseur['suppliers']=$objp->nb_suppliers;
			$this->stats_commande_fournisseur['nb']=$objp->nb;
			$this->stats_commande_fournisseur['rows']=$objp->nb_rows;
			$this->stats_commande_fournisseur['qty']=$objp->qty?$objp->qty:0;
			return 1;
		}
		else
		{
	...
	function load_stats_sending($socid=0,$filtrestatut='')
	{
		...
		{
			$objp=$this->db->fetch_object($result);
			$this->stats_expedition['customers']=$objp->nb_customers;
			$this->stats_expedition['nb']=$objp->nb;
			$this->stats_expedition['rows']=$objp->nb_rows;
			$this->stats_expedition['qty']=$objp->qty?$objp->qty:0;
			return 1;
		}
		else
		{
	...
	function load_stats_contrat($socid=0)
	{
		...
		{
			$objp=$this->db->fetch_object($result);
			$this->stats_contrat['customers']=$objp->nb_customers;
			$this->stats_contrat['nb']=$objp->nb;
			$this->stats_contrat['rows']=$objp->nb_rows;
			$this->stats_contrat['qty']=$objp->qty?$objp->qty:0;
			return 1;
		}
		else
		{
	...
	function load_stats_facture($socid=0)
	{
		...
		{
			$objp=$this->db->fetch_object($result);
			$this->stats_facture['customers']=$objp->nb_customers;
			$this->stats_facture['nb']=$objp->nb;
			$this->stats_facture['rows']=$objp->nb_rows;
			$this->stats_facture['qty']=$objp->qty?$objp->qty:0;
			return 1;
		}
		else
		{
	...
	function load_stats_facture_fournisseur($socid=0)
	{
		...
		{
			$objp=$this->db->fetch_object($result);
			$this->stats_facture_fournisseur['suppliers']=$objp->nb_suppliers;
			$this->stats_facture_fournisseur['nb']=$objp->nb;
			$this->stats_facture_fournisseur['rows']=$objp->nb_rows;
			$this->stats_facture_fournisseur['qty']=$objp->qty?$objp->qty:0;
			return 1;
		}
		else
		{
	...
	function is_sousproduit($fk_parent, $fk_child)
	{
		...
		if ($result)
		{
			...
			{
				$objp = $this->db->fetch_object($result);
				$this->is_sousproduit_qty = $objp->qty;
    ...
    */
	...
	function add_fournisseur($user, $id_fourn, $ref_fourn)
	{
		...
		if ($resql)
		{
			...
			else if ($objp->fk_product == $this->id)
			{
				$this->product_fourn_id = $objp->rowid;
			...
			{
				$this->product_id_already_linked = $objp->fk_product;
	...
	function get_Id_Categorie($id=0)
	{
		global $conf;
		require_once('con_gescom.php'); 
		$list = -1;
		...
		@mysqli_select_db($database_con_gescom, $con_gescom);
		//$result = $this->db->query($sql);
		$result=@mysqli_query($sql, $con_gescom);
		if ($result)
		{
...
*/					'id'=>$desc_pere[0],					// Id product
	...
	{
		require_once(DOL_DOCUMENT_ROOT ."/compta/mesdocs/images.lib.php");
	...
	{
		include_once(DOL_DOCUMENT_ROOT ."/compta/mesdocs/files.lib.php");
	...
	function show_photos_pdf($sdir,$size=0,$nbmax=0,$nbbyrow=5,$showfilename=0,$showaction=0,$maxheight=120)
	{
		global $conf,$user,$langs;

		include_once(DOL_DOCUMENT_ROOT ."/compta/mesdocs/files.lib.php");
		include_once(DOL_DOCUMENT_ROOT ."/compta/mesdocs/images.lib.php");
    				...
    				if (dol_is_file($dir.$file) && preg_match('/(\.jpg|\.bmp|\.gif|\.png|\.tiff)$/i',$dir.$file))
    				{
    					...
    					if ($size == 1) {   // Format vignette
    						...
    						if ($photo_vignette && $imgarray['height'] > $maxheight) {
    							$return.= '';
    							$return.= ''.DOL_URL_ROOT.'/viewimage.php?modulepart=product&file='.urlencode($pdirthumb.$photo_vignette).'';
    						}
    						else {
    							$return.= '';
    							$return.= ''.DOL_URL_ROOT.'/viewimage.php?modulepart=product&file='.urlencode($pdir.$photo).'';
    						}
		...
		if (is_resource($handle))
		{
			while (($file = readdir($handle)) != false)
			{
				...
				if (dol_is_file($dir.$file))
				{
					...
					$objp=array();
					$objp['photo']=$photo;
					if ($photo_vignette && dol_is_file($dirthumb.$photo_vignette)) $objp['photo_vignette']=$photo_vignette;
					else $objp['photo_vignette']="";

					$tabobj[$nbphoto-1]=$objp;
