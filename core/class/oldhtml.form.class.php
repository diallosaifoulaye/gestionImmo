...
class Form
{
	...
	public $cache_motifs_paiements=array();
     ...
     *      @return     int                 Nb of lines loaded, <0 if KO
     */
    public function load_cache_motifs_paiements()
    {
        // phpcs:enable
        global $langs;
        ...
        $this->cache_motifs_paiements = array();

        $sql = "SELECT rowid, code,label, description, statut";
        ...
        if ($resql)
        {
            ...
            while ($i < $num)
            {
                ...
                // Si traduction existe, on l'utilise, sinon on prend le libelle par defaut
                $label=($langs->transnoentitiesnoconv("PaymentTypeShort".$obj->code)!=("PaymentTypeShort".$obj->code)?$langs->transnoentitiesnoconv("PaymentTypeShort".$obj->code):($obj->label!='-'?$obj->label:''));
                $this->cache_motifs_paiements[$obj->id]['rowid'] =$obj->rowid;
                $this->cache_motifs_paiements[$obj->id]['label'] =$obj->label;
                $this->cache_motifs_paiements[$obj->id]['code']=$obj->code;
                $this->cache_motifs_paiements[$obj->id]['description'] =$obj->description;
                $this->cache_motifs_paiements[$obj->id]['statut'] =$obj->statut;
                $i++;
            }

            $this->cache_motifs_paiements = dol_sort_array($this->cache_types_paiements, 'label', 'asc', 0, 0, 1);
     ...
     * 		@return	void
     */
    public function select_motifs_paiements($selected = '', $htmlname = 'paiementmotif', $filtertype = '', $format = 0, $empty = 1, $noadmininfo = 0, $maxlength = 0, $active = 1, $morecss = '')
    {
        // phpcs:enable
        global $langs,$user;
        ...
        $this->load_cache_motifs_paiements();

        print '<select id="select'.$htmlname.'" class="flat selectpaymentmotifs'.($morecss?' '.$morecss:'').'" name="'.$htmlname.'">';
        if ($empty) print '<option value="">&nbsp;</option>';
        foreach($this->cache_motifs_paiements as $id => $arraytypes)
        {
            // If not good status
            if ($active >= 0 && $arraytypes['active'] != $active) continue;
