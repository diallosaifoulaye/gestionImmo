...
class Form
{
		...
		else
		{
			if (GETPOST('action', 'aZ09') == 'edit'.$htmlname)
			{
				...
				if (preg_match('/^(string|safehtmlstring|email)/', $typeofdata))
				{
                    ...
                    if ($action == 'editcustomeraccountancycode' || $action == "editsupplieraccountancycode")
                        $ret .= $this->select_numcompte(($editvalue ? $editvalue : $value), $htmlname, 0, 'style="width: 200px !important;"');
                    else{
                        $tmp = explode(':', $typeofdata);
    ...
    private function select_numcompte($selected='',$htmlname='numcompte',$empty=0, $htmloption='', $condition = null)
    {
        global $conf,$langs, $db;
    ...
    public function select_produits_list($selected = '', $htmlname = 'productid', $filtertype = '', $limit = 20, $price_level = 0, $filterkey = '', $status = 1, $finished = 2, $outputmode = 0, $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = '', $hidepriceinlabel = 0, $warehouseStatus = '')
	{
        ...
        $sql = 'SELECT DISTINCT p.rowid, p.ref, p.label, pe.nature_lot, pe.type, p.description, p.price,pe.prix_commercial, pe.statut_lot, p.barcode, p.price, p.price_ttc, p.price_base_type, nat.label as labelnature,';
        ...
        $sql .= " where  pe.statut_lot = 1";
		$sql.=" AND pe.fk_object=p.rowid";
		if ($filterkey != '')
		{
			$sql .= ' AND (';
    ...
    function select_categorie($selected, $htmlname='fk_categorie',$showempty=0,$fonction='',$classe='minwidth200')
    {
        $out='';
        // On recherche les materiels


      //  $sql=" SELECT c.rowid as id, c.label FROM fdx6_categorie as c ";

        $sql = "SELECT c.rowid as id, c.label";
        $sql.= " FROM ".MAIN_DB_PREFIX."categorie as c";
        $sql.= " WHERE type=0";
        $sql.= " ORDER BY label";
        //echo $sql ;
        dol_syslog(get_class($this)."::select_categorie", LOG_DEBUG);
        $resql=$this->db->query($sql);
        if ($resql)
        {
            $out = '<select id="select'.$htmlname.'" class="flat '.$classe.'" name="'.$htmlname.'"';
            if(!empty($fonction)) $out.= 'onchange="getInfosMateriel()"';
            $out.=  '>';
            ...
            if ($num)
            {
                while ($i < $num)
                {
                    ...
                    {
                        $out.= '<option value="'.$obj->id.'" selected>'.$obj->label.'</option>';
                    }
                    else
                    {
                        $out.= '<option value="'.$obj->id.'">'.$obj->label.'</option>';
                    }
                    $i++;
                }
            }
            ...
            include_once DOL_DOCUMENT_ROOT . '/core/lib/ajax.lib.php';
            $out .= ajax_combobox('select'.$htmlname);
           // $out.= dol_trunc($obj->label,20,'middle');
  ...
  public function showLinkedObjectBlock2($object, $morehtmlright = '', $compatibleImportElementsList = false)
	{
		global $conf, $langs, $hookmanager;
		global $bc, $action;
		...
		$reshook = $hookmanager->executeHooks('showLinkedObjectBlock2', $parameters, $object, $action); // Note that $action and $object may have been modified by hook
		...
		if (empty($reshook))
		{
			...
			print '<td>'.$langs->trans("Ref").'</td>';
			print '<td class="center"></td>';
			print '<td class="center">'.$langs->trans("Dateffff").'</td>';
			print '<td class="right">'.$langs->trans("AmountHTShort").'</td>';
			print '<td class="right">'.$langs->trans("Status").'</td>';
			print '<td></td>';
			print '</tr>';
