...
function print_eldy_menu($db, $atarget, $type_user, &$tabMenu, &$menu, $noout = 0, $mode = '')
{
    ...
    // Dashboard
    $tmpentry = array(
        'enabled'=> ( ! empty($conf->dashboard->enabled) ? 1 : 0),
        'perms'=> (! empty($user->rights->dashboard->lire) ? 1 : 0),
        'module'=>'dashboard'
    );
    $menu_arr[] = array(
        'name' => 'Dashboard',
    ...
    $tabMenu[] = array(
        'name' => 'GRH',
        'link' => "/grh/index.php?mainmenu=members&amp;leftmenu=', $langs->trans('GRH'), 0, $showmode, $atarget, 'grh', ''",
        'title' =>  "GRH",
        'level' => 0,
        'enabled' => $showmode = isVisibleToUserType($type_user, $tmpentry, $listofmodulesforexternal),
        'target' => $atarget,
        ...
        'id' => $id,
        'idsel' => 'grh',
        'classname' =>  $classname = ( $_SESSION["mainmenu"] && $_SESSION["mainmenu"] == "grh" ) ? 'class="tmenusel"' : 'class="tmenu"',
        'prefix' => '',

        'session' => ( ( $_SESSION["mainmenu"] && $_SESSION["mainmenu"] == "grh" ) ? 0 : 1 ),
	...
	$showmode=1;
    $tmpentry = array('enabled' => (!empty($conf->grh->enabled)),
        'perms' => (! empty($user->rights->GRH->lire) ||  ! empty($user->rights->GRH->ecrire)),
        'module' => '');
    $showmode = isVisibleToUserType($type_user, $tmpentry, $listofmodulesforexternal);
    if ($showmode) {
        $classname = "";
        if ($_SESSION["mainmenu"] && $_SESSION["mainmenu"] == "grh") {
            $classname = 'class="tmenu"';
            $_SESSION['idmenu'] = '';
        } else $classname =( $_SESSION["mainmenu"] && $_SESSION["mainmenu"] == "grh" ) ? 'class="tmenu"' : 'class="tmenu"';
        $idsel = 'grh';

        if (empty($noout)) print_start_menu_entry($idsel, $classname, $showmode);
        if (empty($noout)) print_text_menu_entry($langs->trans("GRH"), $showmode, DOL_URL_ROOT . '/grh/index.php?mainmenu=grh&amp;leftmenu=', $id, $idsel, $classname, $atarget);
        if (empty($noout)) print_end_menu_entry($showmode);
        $menu->add('/grh/index.php?mainmenu=members&amp;leftmenu=', $langs->trans("GRH"), 0, $showmode, $atarget, "grh", '');
    }
    if (empty($noout)) {
...
function print_left_eldy_menu($db, $menu_array_before, $menu_array_after, &$tabMenu, &$menu, $noout = 0, $forcemainmenu = '', $forceleftmenu = '', $moredata = null)
{
	...
	if (empty($conf->global->MAIN_MENU_INVERT) && empty($noout) && ! empty($conf->global->MAIN_SHOW_LOGO) && empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER))
	{
        ...
        if ($mainmenu == 'ersa')
        {
            if(!isset($_SESSION['exercice'])){
                $sql  = "SELECT *  FROM " . MAIN_DB_PREFIX . "exercice as e WHERE e.entite_rowid = " . $conf->entity ." ORDER BY e.idexercice DESC";
                $resql = $db->query($sql);
                $exercice = $db->fetch_object($resql);
                $user->exercice = $exercice->idexercice;
                $user->statusExo = $exercice->status;
            }else $user->exercice = $_SESSION['exercice'];
            $langs->load("ersa");
            ...
            print '<div class="menu_contenu" id="menu_contenu_logo">';
            print '<center>
                        <label for="select">Exercice</label>
                        <select name="exercice" id="select" style="width: 50%;" onchange="changeExo(this.value,\''.$_SESSION['newtoken'].'\')">';
            $i = 0;
            while ($i < $num) {
                $oneElem = $db->fetch_object($resql);
                if($user->exercice == null) $user->exercice = $oneElem->idexercice;
                print '<option '. (($user->exercice == $oneElem->idexercice) ? "selected" : "") .' value="'.$oneElem->idexercice.'-'.$oneElem->status.'">'.$oneElem->idexercice.'</option>';
                $i++;
            }
            print '</select>
                </center>'."\n";
            ...
            print '</div>'."\n";
            print '<script>
                function changeExo(val, token) {
                 val = val.split("-");
                    $.post (
                        "/ersa/currentExercice.php",
                        {
                            exo:val[0],
                            status:val[1],
                            token:token
	...
	if ($mainmenu)	// If this is empty, loading hard coded menu and loading personalised menu will fail
	{
		...
		if ($mainmenu == 'companies')
		{
            ...
            $newmenu->add("/societe/list.php?type=f&amp;leftmenu=suppliers", $langs->trans("ListIntervenant"), 1, ($user->rights->fournisseur->lire || $user->rights->supplier_proposal->lire), '', $mainmenu, 'suppliers');
            $newmenu->add("/societe/card.php?leftmenu=suppliers&amp;action=create&amp;type=f", $langs->trans("NewIntervenant"), 2, $user->rights->societe->creer && ($user->rights->fournisseur->lire || $user->rights->supplier_proposal->lire));
		...
		if ($mainmenu == 'billing')
		{
			...
			if (! empty($conf->facture->enabled))
			{
				...
				$newmenu->add("/compta/motif/card.php?id=all&leftmenu=motif&action=list&amp;type=0", $langs->trans("Motifs"), 1, $user->rights->facture->lire, '', $mainmenu, 'customers_bills_payment');
			...
			if (! empty($conf->tax->enabled) || ! empty($conf->salaries->enabled) || ! empty($conf->loan->enabled) || ! empty($conf->banque->enabled))
			{
                ...
                {
                    $langs->load("banks");
                    $newmenu->add("/tableaubord/index/list.php?leftmenu=tax_various&amp;mainmenu=billing", $langs->trans("MenuVariousPayment"), 1, $user->rights->banque->lire, '', $mainmenu, 'tax_various');
                    if ($usemenuhider || empty($leftmenu) || preg_match('/^tax_various/i', $leftmenu)) {
                        $newmenu->add("/compta/bank/various_payment/card.php?leftmenu=tax_various&action=create", $langs->trans("New"), 2, $user->rights->banque->modifier);
                        $newmenu->add("/compta/bank/various_payment/list.php?leftmenu=tax_various", $langs->trans("List"), 2, $user->rights->banque->lire);
                    }
		...
		if ($mainmenu == 'products')
		{
			...
			{
				$newmenu->add("/product/index.php?leftmenu=product&amp;typeprod=0", $langs->trans("lot"), 0, $user->rights->produit->lire, '', $mainmenu, 'product');
				$newmenu->add("/product/card.php?leftmenu=product&amp;action=create&amp;typeprod=0", $langs->trans("NewLot"), 1, $user->rights->produit->creer);
				$newmenu->add("/product/list.php?leftmenu=product&amp;typeprod=0", $langs->trans("List"), 1, $user->rights->produit->lire);
				...
				{
					$newmenu->add("/product/stats/card.php?id=all&leftmenu=stats&typeprod=0", $langs->trans("Statistics"), 1, $user->rights->produit->lire && $user->rights->propale->lire);
				}

                $newmenu->add("/product/nature_lot/card.php?id=all&leftmenu=product&action=list&amp;typeprod=0", $langs->trans("NatureLot"), 1, $user->rights->produit->lire && $user->rights->propale->lire);

                $newmenu->add("/product/type_lot/card.php?id=all&leftmenu=product&action=list&amp;typeprod=0", $langs->trans("TypeLot"), 1, $user->rights->produit->lire && $user->rights->propale->lire);



				$newmenu->add("/product/index_terrain.php?leftmenu=product&amp;type=2", $langs->trans("terrain"), 0, $user->rights->produit->lire, '', $mainmenu, 'product');
				$newmenu->add("/product/card_terrain.php?leftmenu=product&amp;action=create&amp;typeprod=2", $langs->trans("NewTerrain"), 1, $user->rights->produit->creer);
				$newmenu->add("/product/list_terrain.php?leftmenu=product&amp;typeprod=2", $langs->trans("List"), 1, $user->rights->produit->lire);
        ...
        {
            $newmenu->add("/product/index.php?leftmenu=service&amp;typeprod=1", $langs->trans("Services"), 0, $user->rights->service->lire, '', $mainmenu, 'service');
            $newmenu->add("/product/card.php?leftmenu=service&amp;action=create&amp;typeprod=1", $langs->trans("NewService"), 1, $user->rights->service->creer);
            $newmenu->add("/product/list.php?leftmenu=service&amp;typeprod=1", $langs->trans("List"), 1, $user->rights->service->lire);
            if (! empty($conf->propal->enabled) || ! empty($conf->commande->enabled) || ! empty($conf->facture->enabled) || ! empty($conf->fournisseur->enabled) || ! empty($conf->supplier_proposal->enabled))
            {
                $newmenu->add("/product/stats/card.php?id=all&leftmenu=stats&typeprod=1", $langs->trans("Statistics"), 1, $user->rights->service->lire && $user->rights->propale->lire);
            }
            // Categories
            /*if (! empty($conf->categorie->enabled))
            {
                ...
                if (! empty($conf->categorie->enabled))
                {
                    $newmenu->add("/categories/index.php?leftmenu=cat&amp;typeprod=0", $langs->trans("Categories"), 0, $user->rights->categorie->lire, '', $mainmenu, 'cat');
                    $newmenu->add("/categories/card.php?action=create&typeprod=0", $langs->trans("NewCategory"), 1, $user->rights->categorie->creer, '', $mainmenu, 'cat');
                    $newmenu->add("/categories/index.php?leftmenu=cat&amp;typeprod=0", $langs->trans("List"), 1, $user->rights->categorie->lire, '', $mainmenu, 'cat');
                    $newmenu->add("/categories/echeance/card.php?leftmenu=cat&amp;typeprod=0", $langs->trans("Echéances"), 1, $user->rights->produit->lire && $user->rights->propale->lire);
                    //if ($usemenuhider || empty($leftmenu) || $leftmenu=="cat") $newmenu->add("/categories/index.php?leftmenu=cat&amp;type=0", $langs->trans("List"), 1, $user->rights->categorie->lire);
                }
		...
		if ($mainmenu == 'project')
		{
			if (! empty($conf->projet->enabled))
			{
				...
				$newmenu->add("/projet/card.php?leftmenu=projects&action=create".($search_project_user?'&search_project_user='.$search_project_user:''), $langs->trans("Nouveau projet"), 1, $user->rights->projet->creer);
				$newmenu->add("/projet/cardopp.php?leftmenu=projects&action=create".($search_project_user?'&search_project_user='.$search_project_user:''), $langs->trans("Nouvelle opportunité"), 1, $user->rights->projet->creer);
				...
				elseif ($conf->global->PROJECT_USE_OPPORTUNITIES == 1)
				{
					...
					$newmenu->add('/projet/listopp.php?mainmenu=project&amp;leftmenu=list&search_opp_status=openedopp&search_status=99&contextpage=lead', $langs->trans("ListOpenLeads"), 2, $showmode);
                    ...
                    $newmenu->add('/projet/poid_op.php?mainmenu=project&amp;leftmenu=list&action=list', $langs->trans("Poids Opportunité "), 2, $showmode);
                }
        ...
        {
            if (! empty($conf->dashboard->enabled))
            {
                $langs->load("projects");
                ...
                $tmpentry = array(
                    'enabled'=>(! empty($conf->dashboard->enabled)),
                    'perms'=>(! empty($user->rights->dashboard->lire)),
                    'module'=>'dashboard'
                );
                $showmode=isVisibleToUserType($type_user, $tmpentry, $listofmodulesforexternal);
		...
		*/
        ...
        {
            $langs->load("grh");
            $sousleftmenu=$_GET["sousleftmenu"];
            $_SESSION["sousleftmenu"]=$sousleftmenu;

            // Customers invoices
            if (! empty($conf->grh->enabled))
            {
                //$langs->load("bills");
                $leftmenuArray = array("grh_rh","grh_rh_tmensuels","grh_rh_tannuels","grh_rh_tjournaliers","grh_rh_parametres","grh_rh_etats","grh_rh_historiques","grh_rh_employes","grh_paie", "grh_paie_parametres","grh_paie_tjournaliers","grh_paie_tmensuels", "grh_paie_historiques", "grh_paie_etats","grh_paie_tannuels");
                $leftmenuRHArray = array("grh_rh","grh_rh_tmensuels","grh_rh_tannuels","grh_rh_tjournaliers","grh_rh_parametres","grh_rh_etats","grh_rh_historiques","grh_rh_employes","grh_paie");
                $leftmenuPaieArray = array("grh_rh","grh_paie","grh_paie_parametres","grh_paie_tjournaliers","grh_paie_tmensuels", "grh_paie_tannuels", "grh_paie_historiques", "grh_paie_etats");

                $newmenu->add("/grh/ressourceshumaines.php?leftmenu=grh_rh",$langs->trans("Ressources Humaines"),0,$user->rights->GRH->lire, '', $mainmenu, 'grh');
                if (empty($leftmenu) || in_array($leftmenu,$leftmenuRHArray)) $newmenu->add("/grh/parametresrh.php?leftmenu=grh_rh_parametres",$langs->trans("Paramètres"),1,$user->rights->GRH->ecrire);
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/service.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Services"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/poste_emploi.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Emplois"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/qualification.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Qualifications"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/categorie_emploi.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Categories"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/profil.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Profils"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/diplome.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Diplômes"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/situationfamille.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Situation de Famille"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/pays.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Pays"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/nationalite.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Nationalites"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/majorationconges.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Majorations Congés"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/tauxheuregrh.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Barème Heures"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/groupe_emploi.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Groupe Employés"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/retraite.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Barèmes Retraite"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/licenciement.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("Barèmes Licenciement"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');
                if ($leftmenu=="grh_rh_parametres") $newmenu->add("/grh/typemaj.php?leftmenu=grh_rh_parametres&amp;action=list",$langs->trans("MAJ Administratives"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_parametres');



                if (empty($leftmenu) || in_array($leftmenu,$leftmenuRHArray) ) $newmenu->add("/grh/tjournaliersrh.php?leftmenu=grh_rh_tjournaliers",$langs->trans("Actes Journaliers"),1,$user->rights->GRH->lire);
                if ($leftmenu=="grh_rh_tjournaliers" || $leftmenu=="grh_rh_employes") $newmenu->add("/grh/personnel.php?leftmenu=grh_rh_employes&amp;action=list",$langs->trans(" Les Employés"),2,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers" || $leftmenu=="grh_rh_employes") $newmenu->add("/grh/personnel.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("MAJ Employés"),3,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers" || $leftmenu=="grh_rh_employes") $newmenu->add("/grh/personnel_enfant.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("MAJ Enfants"),3,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers" || $leftmenu=="grh_rh_employes" ) $newmenu->add("/grh/posseder_diplome.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("Diplômes Employé"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers" || $leftmenu=="grh_rh_employes" ) $newmenu->add("/grh/etreclasse.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("Groupes Emploi"),3,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers" ) $newmenu->add("/grh/priseservicegrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list&amp;type=c",$langs->trans("Prise de Service"),3,$user->rights->GRH->decision_responsable,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers" ) $newmenu->add("/grh/formationgrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("Formations"),3,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                /*if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/formationgrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list&amp;type=c",$langs->trans("Demande"),4,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/formationgrh.php?leftmenu=grh_rh_tjournaliers&amp;action=validate&amp;type=v",$langs->trans("Validation"),4,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                */if ($leftmenu=="grh_rh_tjournaliers" ) $newmenu->add("/grh/nominationgrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list&amp;type=c",$langs->trans("Nominations"),3,$user->rights->GRH->decision_responsable,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/politiquesocialegrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list&amp;type=c",$langs->trans("Politique Sociale"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/hygienesecuritegrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list&amp;type=c",$langs->trans("Hygiène&Sécurité"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/actemedicalgrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list&amp;type=c",$langs->trans("Actes Médicaux"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/organisationtempsgrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list&amp;type=c",$langs->trans("Temps de Travail"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');

                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/cessationdefinitivegrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("Décès"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/cessation_service_contrat.php?leftmenu=grh_rh_tjournaliers&amp;action=view_paysheet",$langs->trans("Fin Contrat"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');


                if ($leftmenu=="grh_rh_tjournaliers" || $leftmenu=="grh_rh_employes") $newmenu->add("/grh/demandepostegrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("Recrutements"),2,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/demandepostegrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("Pourvoi Poste"),3,$user->rights->GRH->decision_responsable,'','grh_rh','grh_rh_tjournaliers');
                /*if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/demandepostegrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list&amp;type=v",$langs->trans("Valider Demande"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                */if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/postulantgrh.php?leftmenu=grh_rh_tjournaliers&amp;action=add&amp;type=c",$langs->trans("Postuler"),3,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/postulantgrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("Suivi"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                /*if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/postulantgrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("MAJ Postulants"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');*/


                if ($leftmenu=="grh_rh_tjournaliers" || $leftmenu=="grh_rh_employes") $newmenu->add("/grh/permissiongrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("Absences"),2,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/permissiongrh.php?leftmenu=grh_rh_tjournaliers&amp;action=add&amp;type=c",$langs->trans("Permission"),3,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/absence.php?leftmenu=grh_rh_tjournaliers&amp;action=add&amp;type=c",$langs->trans("Autres"),3,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers" || $leftmenu=="grh_rh_employes") $newmenu->add("/grh/demande_conge.php?leftmenu=grh_rh_tjournaliers&amp;action=list&amp;type=l",$langs->trans("Congés"),2,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/demande_conge.php?leftmenu=grh_rh_tjournaliers&amp;action=add&amp;type=c",$langs->trans("Demandes"),3,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/demande_conge.php?leftmenu=grh_rh_tjournaliers&amp;action=list&amp;type=m",$langs->trans("Validation"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/demande_conge.php?leftmenu=grh_rh_tjournaliers&amp;action=come_back_towork&amp;type=m",$langs->trans("Reprises"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers" || $leftmenu=="grh_rh_employes") $newmenu->add("/grh/suspensiongrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("Suspension Contrats"),2,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/suspensiongrh.php?leftmenu=grh_rh_tjournaliers&amp;action=add&amp;type=c",$langs->trans("Demandes"),3,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/suspensiongrh.php?leftmenu=grh_rh_tjournaliers&amp;action=validate&amp;type=v",$langs->trans("Validation"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers" || $leftmenu=="grh_rh_employes") $newmenu->add("/grh/demissiongrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list&amp;type=l",$langs->trans("Démission"),2,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/demissiongrh.php?leftmenu=grh_rh_tjournaliers&amp;action=add&amp;type=c",$langs->trans("Dépôt"),3,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers") $newmenu->add("/grh/demissiongrh.php?leftmenu=grh_rh_tjournaliers&amp;action=validate&amp;type=v",$langs->trans("Validation"),3,$user->rights->GRH->decision_responsable||$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_tjournaliers" || $leftmenu=="grh_rh_employes") $newmenu->add("/grh/actelicenciementgrh.php?leftmenu=grh_rh_tjournaliers&amp;action=list",$langs->trans("Licenciement"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');

                if (empty($leftmenu) || in_array($leftmenu,$leftmenuRHArray)) $newmenu->add("/grh/tmensuelsrh.php?leftmenu=grh_rh_tmensuels",$langs->trans("Actes Mensuels"),1,$user->rights->GRH->lire);
                if ($leftmenu=="grh_rh_tmensuels") $newmenu->add("/grh/gestionretraite.php?leftmenu=grh_rh_tmensuels&amp;action=list",$langs->trans("Retraite"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if (empty($leftmenu) || in_array($leftmenu,$leftmenuRHArray)) $newmenu->add("/grh/tannuelsrh.php?leftmenu=grh_rh_tannuels",$langs->trans("Actes Annuels"),1,$user->rights->GRH->lire);
                if ($leftmenu=="grh_rh_tannuels") $newmenu->add("/grh/bilan_social.php?leftmenu=grh_rh_tannuels&amp;action=add&amp;type=c",$langs->trans("Bilan Social"),2,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if (empty($leftmenu) || in_array($leftmenu,$leftmenuRHArray)) $newmenu->add("/grh/etatsrh.php?leftmenu=grh_rh_etats",$langs->trans("Les Etats"),1,$user->rights->GRH->lire);

                if ($leftmenu=="grh_rh_etats") $newmenu->add("/grh/depart_retraite.php?leftmenu=grh_rh_etats&amp;action=view_paysheet",$langs->trans("Départs Retraite"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_etats") $newmenu->add("/grh/licencie.php?leftmenu=grh_rh_etats&amp;action=view_paysheet",$langs->trans("Licenciés"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_etats") $newmenu->add("/grh/demissionaire.php?leftmenu=grh_rh_etats&amp;action=view_paysheet",$langs->trans("Démissionaires"),2,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_etats") $newmenu->add("/grh/suspendus.php?leftmenu=grh_rh_etats&amp;action=view_paysheet",$langs->trans("Suspension Contrat"),2,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');
                if ($leftmenu=="grh_rh_etats") $newmenu->add("/grh/attestation_travail.php?leftmenu=grh_rh_etats",$langs->trans("Attestation Travail"),2,$user->rights->GRH->lire,'','grh_rh','grh_rh_tjournaliers');

                if (empty($leftmenu) || in_array($leftmenu,$leftmenuRHArray)) $newmenu->add("/grh/historiquesrh.php?leftmenu=grh_rh_historiques",$langs->trans("Les Historiques"),1,$user->rights->GRH->lire);
                if ($leftmenu=="grh_rh_historiques") $newmenu->add("/grh/historiquepersonnel.php?leftmenu=grh_rh_historiques&amp;action=add&amp;type=c",$langs->trans("Historique d'un employé"),2,$user->rights->GRH->lire,'','grh_rh','grh_rh_historiques');



                $newmenu->add("/grh/paie.php?leftmenu=grh_paie",$langs->trans("Paie"),0,$user->rights->GRH->lire, '', $mainmenu, 'grh');
                if (empty($leftmenu) || in_array($leftmenu,$leftmenuPaieArray)) $newmenu->add("/grh/parametrespaie.php?leftmenu=grh_paie_parametres",$langs->trans("Paramètres"),1,$user->rights->GRH->lire);
                if ($leftmenu=="grh_paie_parametres") $newmenu->add("/grh/calendrier_paie.php?leftmenu=grh_paie_parametres&amp;action=list",$langs->trans("Calendriers de Paie"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_parametres');
                if ($leftmenu=="grh_paie_parametres") $newmenu->add("/grh/rubrique.php?leftmenu=grh_paie_parametres&amp;action=list",$langs->trans("Rubriques de Paie"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_parametres');
                if ($leftmenu=="grh_paie_parametres") $newmenu->add("/grh/assiette.php?leftmenu=grh_paie_parametres&amp;action=list",$langs->trans("Assiettes de Paie"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_parametres');
                if ($leftmenu=="grh_paie_parametres") $newmenu->add("/grh/bareme_impot.php?leftmenu=grh_paie_parametres&amp;action=list",$langs->trans("Barèmes Impôts"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_parametres');
                if ($leftmenu=="grh_paie_parametres") $newmenu->add("/grh/abattement.php?leftmenu=grh_paie_parametres&amp;action=list",$langs->trans("Abattements"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_parametres');
                if ($leftmenu=="grh_paie_parametres") $newmenu->add("/grh/banque.php?leftmenu=grh_paie_parametres&amp;action=list",$langs->trans("Banques"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_parametres');
                if ($leftmenu=="grh_paie_parametres") $newmenu->add("/grh/compte.php?leftmenu=grh_paie_parametres&amp;action=list",$langs->trans("Comptes Bancaires"),2,$user->rights->GRH->lire,'','grh_paie','grh_paie_parametres');

                if ($leftmenu=="grh_paie_parametres") $newmenu->add("/grh/emarger.php?leftmenu=grh_paie_parametres&amp;action=list",$langs->trans("Déversements"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_parametres');


                if (empty($leftmenu) || in_array($leftmenu,$leftmenuPaieArray)) $newmenu->add("/grh/tjournalierspaie.php?leftmenu=grh_paie_tjournaliers",$langs->trans("Actes Journaliers"),1,$user->rights->GRH->lire);
                if ($leftmenu=="grh_paie_tjournaliers") $newmenu->add("/grh/variables.php?leftmenu=grh_paie_tjournaliers&amp;action=list",$langs->trans("Saisie Variables"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_tjournaliers');
                if ($leftmenu=="grh_paie_tjournaliers") $newmenu->add("/grh/heure.php?leftmenu=grh_paie_tjournaliers&amp;action=list",$langs->trans("Heures Supplémentaires"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_tjournaliers');

                if (empty($leftmenu) ||in_array($leftmenu,$leftmenuPaieArray)) $newmenu->add("/grh/tmensuelspaie.php?leftmenu=grh_paie_tmensuels",$langs->trans("Actes Mensuels"),1,$user->rights->GRH->lire);
                if ($leftmenu=="grh_paie_tmensuels") $newmenu->add("/grh/pointage_employe.php?leftmenu=grh_paie_tmensuels&amp;action=list&amp;type=l",$langs->trans("Les Pointages"),2,$user->rights->GRH->lire,'','grh_paie','grh_paie_tmensuels');
                if ($leftmenu=="grh_paie_tmensuels") $newmenu->add("/grh/pointage_employe.php?leftmenu=grh_paie_tmensuels&amp;action=add&amp;type=ce",$langs->trans("Générer"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tmensuels');
                if ($leftmenu=="grh_paie_tmensuels") $newmenu->add("/grh/pointage_employe.php?leftmenu=grh_paie_tmensuels&amp;action=list&amp;type=m",$langs->trans("Modifier"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tmensuels');
                if ($leftmenu=="grh_paie_tmensuels") $newmenu->add("/grh/heure.php?leftmenu=grh_paie_tmensuels&amp;action=list",$langs->trans("La Paie"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_tmensuels');
                if ($leftmenu=="grh_paie_tmensuels") $newmenu->add("/grh/traitementpaie.php?leftmenu=grh_paie_tmensuels&amp;action=list",$langs->trans("Lancer"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tmensuels');
                if ($leftmenu=="grh_paie_tmensuels") $newmenu->add("/grh/consultation_bulletin.php?leftmenu=grh_paie_tmensuels&amp;action=view_paysheet",$langs->trans("Consulter"),3,$user->rights->GRH->lire,'','grh_rh','grh_rh_tmensuels');
                if ($leftmenu=="grh_paie_tmensuels") $newmenu->add("/grh/annulation_paie.php?leftmenu=grh_paie_tmensuels&amp;action=list",$langs->trans("Annuler"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tmensuels');
                if ($leftmenu=="grh_paie_tmensuels") $newmenu->add("/grh/cloturepaie.php?leftmenu=grh_paie_tmensuels",$langs->trans("Clôturer"),3,$user->rights->GRH->ecrire,'','grh_rh','grh_rh_tmensuels');



                if (empty($leftmenu) || in_array($leftmenu,$leftmenuPaieArray)) $newmenu->add("/grh/tannuelspaie.php?leftmenu=grh_paie_tannuels",$langs->trans("Actes Annuels"),1,$user->rights->GRH->lire);
                if ($leftmenu=="grh_paie_tannuels") $newmenu->add("/grh/consultation_declaration_css.php?leftmenu=grh_paie_tannuels&amp;action=view_paysheet",$langs->trans("Déclaration CSS"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_tannuels');
                if ($leftmenu=="grh_paie_tannuels") $newmenu->add("/grh/consultation_declaration_impot.php?leftmenu=grh_paie_tannuels&amp;action=view_paysheet",$langs->trans("Déclaration Impôt"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_tannuels');
                if ($leftmenu=="grh_paie_tannuels") $newmenu->add("/grh/consultation_declaration_ipres.php?leftmenu=grh_paie_tannuels&amp;action=view_paysheet",$langs->trans("Déclaration IPRES"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_tannuels');
                if (empty($leftmenu) || in_array($leftmenu,$leftmenuPaieArray)) $newmenu->add("/grh/etatspaie.php?leftmenu=grh_paie_etats",$langs->trans("Les Etats"),1,$user->rights->GRH->lire);
                if ($leftmenu=="grh_paie_etats") $newmenu->add("/grh/consultation_livre_paie.php?leftmenu=grh_paie_etats&amp;action=search_and_found",$langs->trans("Livre de Paie"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_etats');
                if ($leftmenu=="grh_paie_etats") $newmenu->add("/grh/consultation_liste_paiements.php?leftmenu=grh_paie_etats&amp;action=search_and_found",$langs->trans("Liste De Virements"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_etats');
                if ($leftmenu=="grh_paie_etats") $newmenu->add("/grh/consultation_bordereau_recap.php?leftmenu=grh_paie_etats&amp;action=search_and_found",$langs->trans("Bordereau de Recap"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_etats');
                if ($leftmenu=="grh_paie_etats") $newmenu->add("/grh/consultation_retenue_solde.php?leftmenu=grh_paie_etats&amp;action=search_and_found",$langs->trans("Retenues Solde"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_etats');
                if ($leftmenu=="grh_paie_etats") $newmenu->add("/grh/consultation_retenue_diverse.php?leftmenu=grh_paie_etats&amp;action=search_and_found",$langs->trans("Retenues Diverses"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_etats');
                if ($leftmenu=="grh_paie_etats") $newmenu->add("/grh/consultation_echeancier.php?leftmenu=grh_paie_etats&amp;action=search_and_found",$langs->trans("Echéancier"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_etats');
                if ($leftmenu=="grh_paie_etats") $newmenu->add("/grh/consultation_suivi_effectif.php?leftmenu=grh_paie_etats&amp;action=search_and_found",$langs->trans("Suivi Effectif"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_etats');
                if ($leftmenu=="grh_paie_etats") $newmenu->add("/grh/consultation_declaration_mensuelle.php?leftmenu=grh_paie_etats&amp;action=search_and_found",$langs->trans("CSS Mensuelle"),2,$user->rights->GRH->ecrire,'','grh_paie','grh_paie_etats');
                if (empty($leftmenu) || in_array($leftmenu,$leftmenuPaieArray)) $newmenu->add("/grh/historiquespaie.php?leftmenu=grh_paie_historiques",$langs->trans("Les Historiques"),1,$user->rights->GRH->lire);
                if ($leftmenu=="grh_paie_historiques") $newmenu->add("/grh/consultation_bulletin_annuel.php?leftmenu=grh_paie_historiques&amp;action=view_paysheet",$langs->trans("Bulletin Annuel"),2,$user->rights->GRH->lire,'','grh_paie','grh_paie_historiques');
                if ($leftmenu=="grh_paie_historiques") $newmenu->add("/grh/consultation_bulletin_histo.php?leftmenu=grh_paie_historiques&amp;action=view_paysheet",$langs->trans("Bulletin "),2,$user->rights->GRH->lire,'','grh_paie','grh_paie_historiques');
                if ($leftmenu=="grh_paie_historiques") $newmenu->add("/grh/consultation_bulletin_histo.php?leftmenu=grh_paie_historiques&amp;action=view_paysheet",$langs->trans("Bulletin "),2,$user->rights->GRH->lire,'','grh_paie','grh_paie_historiques');
