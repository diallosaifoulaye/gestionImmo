...
/*
 *function GetgrilleParRowid($rowid)
{
    global $db;
    $sql = "SELECT * FROM " .MAIN_DB_PREFIX."categorie  WHERE rowid = ".$rowid;
    $resql = $db->query($sql);
...
if ($action == 'voirGrille') {
    $ligne = GetgrilleParRowid($id);
    print '<input type="hidden" name="rowid" value="'.$ligne->rowid.'">';
//$sql = 'SELECT DISTINCT p.rowid, p.ref, p.label, p.barcode, p.price, p.price_ttc, p.price_base_type,';
    $sql = 'SELECT DISTINCT p.rowid, c.rowid p.ref, p.label, pe.nature_lot, pe.type_lot, pe.prix_gestion,pe.prix_commercial, pe.statut_lot, pe.type_lot, p.barcode, p.price, p.price_ttc, p.price_base_type, nat.label as labelnature,';
    $sql.= ' p.fk_product_type, p.tms as datem,';
    $sql.= ' p.duration, p.tosell, p.tobuy, p.seuil_stock_alerte';
    $sql.= ' FROM '.MAIN_DB_PREFIX.'product as p, '.MAIN_DB_PREFIX.'product_extrafields as pe, '.MAIN_DB_PREFIX.'nature_lot as nat, '.MAIN_DB_PREFIX.'categorie as c';
    $sql .= " WHERE c.entity IN (" . getEntity('category') . ")";
    $sql .= " AND c.rowid = " . (int) $rowid;
