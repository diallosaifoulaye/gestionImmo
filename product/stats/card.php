...
if (empty($id) & empty($ref))
{
    ...
    $head[$h][0] = DOL_URL_ROOT.'/product/stats/card.php'.($typeprod != ''?'?type='.$typeprod:'');
    $head[$h][1] = $langs->trans("Chart");
    $head[$h][2] = 'chart';
    $h++;

	$title = $langs->trans("ListProductServiceByPopularity");
    if ((string) $typeprod == '1') {
    	$title = $langs->trans("ListServiceByPopularity");
    }
    if ((string) $typeprod == '0') {
    	$title = $langs->trans("ListProductByPopularity");
    }

    $head[$h][0] = DOL_URL_ROOT.'/product/popuprop.php'.($typeprod != ''?'?type='.$typeprod:'');
    $head[$h][1] = $title;
...
if ($result || empty($id))
{
	...
	if (empty($id))
	{
		...
		print $form->selectarray('type', $array, $typeprod);
		print '</td></tr>';
	...
	if ($mode == 'bynumber') print '<a href="'.$_SERVER["PHP_SELF"].'?id='.(GETPOST('id')?GETPOST('id'):$object->id).($typeprod != '' ? '&type='.$typeprod:'').'&mode=byunit&search_year='.$search_year.'">';
	...
	if ($mode == 'byunit') print '<a href="'.$_SERVER["PHP_SELF"].'?id='.(GETPOST('id')?GETPOST('id'):$object->id).($typeprod != '' ? '&type='.$typeprod:'').'&mode=bynumber&search_year='.$search_year.'">';
	else print img_picto('', 'tick').' ';
	...
	if($conf->propal->enabled) {
		...
		'file' => $object->id.'/propal12m'.((string) $typeprod != '' ? '_type'.$typeprod : '').'_'.$mode.($search_year?'_year'.$search_year:'').'.png',
	...
	if($conf->supplier_proposal->enabled) {
		...
		'file' => $object->id.'/proposalssuppliers12m'.((string) $typeprod != '' ? '_type'.$typeprod : '').'_'.$mode.($search_year?'_year'.$search_year:'').'.png',
	...
	if($conf->order->enabled) {
		...
		'file' => $object->id.'/orders12m'.((string) $typeprod != '' ? '_type'.$typeprod : '').'_'.$mode.($search_year?'_year'.$search_year:'').'.png',
	...
	if($conf->fournisseur->enabled) {
		...
		'file' => $object->id.'/orderssuppliers12m'.((string) $typeprod != '' ? '_type'.$typeprod : '').'_'.$mode.($search_year?'_year'.$search_year:'').'.png',
	...
	if($conf->facture->enabled) {
		...
		'file' => $object->id.'/invoices12m'.((string) $typeprod != '' ? '_type'.$typeprod : '').'_'.$mode.($search_year?'_year'.$search_year:'').'.png',
		...
		'file' => $object->id.'/invoicessuppliers12m'.((string) $typeprod != '' ? '_type'.$typeprod : '').'_'.$mode.($search_year?'_year'.$search_year:'').'.png',
		...
		if (! $mesg)
		{
			foreach($graphfiles as $key => $val)
			{
				...
				else
				{
					...
					if ($key == 'propal')             $graph_data = $object->get_nb_propal($socid, $mode, ((string) $typeprod != '' ? $typeprod : -1), $search_year, $morefilters);
					if ($key == 'orders')             $graph_data = $object->get_nb_order($socid, $mode, ((string) $typeprod != '' ? $typeprod : -1), $search_year, $morefilters);
					if ($key == 'invoices')           $graph_data = $object->get_nb_vente($socid, $mode, ((string) $typeprod != '' ? $typeprod : -1), $search_year, $morefilters);
					if ($key == 'proposalssuppliers') $graph_data = $object->get_nb_propalsupplier($socid, $mode, ((string) $typeprod != '' ? $typeprod : -1), $search_year, $morefilters);
					if ($key == 'invoicessuppliers')  $graph_data = $object->get_nb_achat($socid, $mode, ((string) $typeprod != '' ? $typeprod : -1), $search_year, $morefilters);
					if ($key == 'orderssuppliers')    $graph_data = $object->get_nb_ordersupplier($socid, $mode, ((string) $typeprod != '' ? $typeprod : -1), $search_year, $morefilters);
	...
	if ( count($graphfiles)>0)
	{
		foreach($graphfiles as $key => $val)
		{
			...
			$linktoregenerate='<a class="reposition" href="'.$_SERVER["PHP_SELF"].'?id='.(GETPOST('id')?GETPOST('id'):$object->id).((string) $typeprod != ''?'&type='.$typeprod:'').'&action=recalcul&mode='.$mode.'&search_year='.$search_year.'&search_categ='.$search_categ.'">'.img_picto($langs->trans("ReCalculate").' ('.$dategenerated.')', 'refresh').'</a>';
