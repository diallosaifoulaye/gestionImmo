...
if (empty($reshook))
{
    ...
    if ($action == 'add' && $usercancreate)
    {
        ...
        if (! $error)
        {
            ...
            $object->length =GETPOST("options_niveau");
...
else
{
	...
	if ($action == 'create' && $usercancreate)
    {
        ...
        $statutarray=array('1' => $langs->trans("OnSell"), '0' => $langs->trans("NotOnSell"),'2' => $langs->trans("ReserveSell"),'3' => $langs->trans("VenduSell"));
       ...
       /* print '<tr><td>'.$langs->trans("PublicUrl").'</td><td colspan="3">';
        ...
        if ($type != 1)	// Nature, Weight and volume only applies to products and not to services
        {
           ...
           // print '<tr><td>Niveau</td><td>';
            //print '<input type="hidden" name="size" size="4" value="'.$_POST["size"].'">';
            //print $formproduct->select_measuring_units("size_units","size");
           // Select_NiveauProduct('size',$_POST["size"]);
            //print '</td></tr>';
            // Weight
          ...
          //  print '<tr><td class="fieldrequired">'.$langs->trans("Status").' ('.$langs->trans("Sell").')</td><td colspan="3">';
           // print '<select class="flat" name="statut">';
            if ($object->status)
            {
                ...
                print '<option value="2" >'.$langs->trans("ReserveSell").'</option>';
                print '<option value="3">'.$langs->trans("VenduSell").'</option>';*/

                print '<tr><td class="fieldrequired">'.$langs->trans("Status").' ('.$langs->trans("Sell").')</td><td colspan="3">';
                $statutarray=array('1' => $langs->trans("OnSell"), '0' => $langs->trans("NotOnSell"),'2' => $langs->trans("ReserveSell"),'3' => $langs->trans("VenduSell"));
                print $form->selectarray('statut', $statutarray,$object->status);
                print '</td></tr>';
            ...
            else
            {
                print '<tr><td class="fieldrequired">'.$langs->trans("Status").' ('.$langs->trans("Sell").')</td><td colspan="3">';
                $statutarray=array('1' => $langs->trans("OnSell"), '0' => $langs->trans("NotOnSell"),'2' => $langs->trans("ReserveSell"),'3' => $langs->trans("VenduSell"));
                print $form->selectarray('statut', $statutarray,0);
                print '</td></tr>';
           ...
           // print '<select class="flat" name="statut_buy">';
            if ($object->status_buy)
            {
               /* print '<option value="1" selected>'.$langs->trans("ProductStatusOnBuy").'</option>';
                ...
                print '<tr><td class="fieldrequired">'.$langs->trans("Status").' ('.$langs->trans("Buy").')</td><td colspan="3">';
                $statutarray=array('1' => $langs->trans("ProductStatusOnBuy"), '0' => $langs->trans("ProductStatusNotOnBuy"));
                print $form->selectarray('statut_buy', $statutarray, $object->status_buy);
                print '</td></tr>';
            }
            else
            {
                print '<tr><td class="fieldrequired">'.$langs->trans("Status").' ('.$langs->trans("Buy").')</td><td colspan="3">';
                $statutarray=array('1' => $langs->trans("ProductStatusOnBuy"), '0' => $langs->trans("ProductStatusNotOnBuy"));
                print $form->selectarray('statut_buy', $statutarray, 0);
                print '</td></tr>';
...
//            print '</table>';
//            print '</div>';
            print '<div class="fichecenter"><div class="fichecenter">';
            ...
            if ($object->isService())
            {
                ...
                print '</td></tr>';
                // Other attributes
                $parameters=array('colspan' => ' colspan="'.(0+(($showphoto||$showbarcode)?1:0)).'"');
                include DOL_DOCUMENT_ROOT . '/core/tpl/extrafields_view.tpl.php';
            }
            else
            {
                // Nature
                print '<tr><td class="titlefield">'.$langs->trans("Nature").'</td><td colspan="0">';
                ...
                // Weight
               /* print '<tr><td class="titlefield">'.$langs->trans("Weight").'</td><td colspan="0">';
                if ($object->weight != '')
                {
              ...
              /*  if (empty($conf->global->PRODUCT_DISABLE_SIZE))
                {
                    // Length
                    print '<tr><td>'.$langs->trans("Length").' x '.$langs->trans("Width").' x '.$langs->trans("Height").'</td><td colspan="0">';
                ...
                {
                    // Surface
                    print '<tr><td>'.$langs->trans("Surface").'</td><td colspan="0">';
                    if ($object->surface != '')
                    {
              ...
              /*  if (empty($conf->global->PRODUCT_DISABLE_VOLUME))
                {
                    // Volume
                    print '<tr><td>'.$langs->trans("Volume").'</td><td colspan="0">';
                    if ($object->volume != '')
                    {
    	    ...
    	    if (! $object->isService() && empty($conf->global->PRODUCT_DISABLE_CUSTOM_INFO))
        	{
	            print '<tr><td>'.$langs->trans("CustomCode").'</td><td colspan="0">'.$object->customcode.'</td>';

            	// Origin country code
            	print '<tr><td>'.$langs->trans("CountryOrigin").'</td><td colspan="0">'.getCountry($object->country_id, 0, $db).'</td>';
        	}


            // Description
            print '<tr><td class="tdtop">'.$langs->trans("Description").'</td><td colspan="0">'.(dol_textishtml($object->description)?$object->description:dol_nl2br($object->description, 1, true)).'</td></tr>';
			...
			{
    			print '<!-- show Note --> '."\n";
                print '<tr><td class="tdtop">'.$langs->trans("NotePrivate").'</td><td colspan="'.(0+(($showphoto||$showbarcode)?1:0)).'">'.(dol_textishtml($object->note_private)?$object->note_private:dol_nl2br($object->note_private, 1, true)).'</td></tr>'."\n";
                print '<!-- End show Note --> '."\n";
			}
