...
$desc = GETPOST('desc','text');
$price = GETPOST('price','int');
$price_ttc =((int)$price*18)/100;
//var_dump($price_ttc);

$prix_gestion = GETPOST('options_prix_gestion','int');
$typeprod = (GETPOST('typeprod', 'int') !== '') ? GETPOST('typeprod', 'int') : Product::TYPE_PRODUCT;
...
// fetch optionals attributes and labels	ProductStatusNotOnBuy
...
$usercanread = (($object->type == Product::TYPE_PRODUCT && $user->rights->produit->lire) ||($object->type == Product::TYPE_TERRAIN && $user->rights->produit->lire) || ($object->type == Product::TYPE_SERVICE && $user->rights->service->lire));
$usercancreate = (($object->type == Product::TYPE_PRODUCT && $user->rights->produit->creer)||($object->type == Product::TYPE_TERRAIN && $user->rights->produit->lire) || ($object->type == Product::TYPE_SERVICE && $user->rights->service->creer));
//var_dump($usercancreate);die();
$usercandelete = (($object->type == Product::TYPE_PRODUCT && $user->rights->produit->supprimer) ||($object->type == Product::TYPE_TERRAIN && $user->rights->produit->lire)|| ($object->type == Product::TYPE_SERVICE && $user->rights->service->supprimer));
...
if (empty($reshook))
{
    ...
    {
        $typeproduct = GETPOST('type', 'int');
        //var_dump($typeproduct);die();
        if ($typeproduct==1){
            ...
            if (empty($ref))
            {
                ...
                header("location: ".DOL_URL_ROOT."/product/card.php?leftmenu=service&action=create&typeprod=1");
                $error++;
            }
            if (empty($label))
            {
            ...
            if (empty($price))
            {
                setEventMessages($langs->trans('ErrorFieldRequired', $langs->transnoentities('Prix du service')), null, 'errors');
                header("location: ".DOL_URL_ROOT."/product/card.php?leftmenu=service&action=create&typeprod=1");
                $error++;
            }
            if (!$error)
            {
                ...
                if ($result ) {
                    $id = $db->last_insert_id(MAIN_DB_PREFIX."product");
                    if ($id > 0) {
                        $statut_lot = 1;
                        $sql1 = "INSERT INTO ".MAIN_DB_PREFIX."product_extrafields (";
                        $sql1.= "fk_object";
                       // $sql1.= ", 	prix_gestion";
                        $sql1.= ", statut_lot";
                        $sql1.= ") VALUES (";
                       ...
                       // $sql1.= ", '".$prix_gestion."'";
                        ...
                        if ($result1) {
                            header("location: ".DOL_URL_ROOT."/product/list.php?leftmenu=service&typeprod=1");
                        }
                        else
                        {
                            $sql2 = "DELETE FROM " . MAIN_DB_PREFIX . "product  WHERE rowid = '$id'";
                            $resql = $db->query($sql2);
                            $error++;
                            $error= $db->lasterror();
                            header("location: ".DOL_URL_ROOT."/product/card.php?leftmenu=service&action=create&typeprod=1");
                ...
                {
                    setEventMessage("Cette référence existe déja", 'errors');
                    header("location: ".DOL_URL_ROOT."/product/card.php?leftmenu=service&action=create&typeprod=1");
                    die;
                }
            ...
            if (!GETPOST('categories', 'alphanohtml'))
            {
    ...
    // Update a product or service
    if ($action == 'createsousciption' && $usercancreate)
    {
        if (GETPOST('cancel', 'alpha'))
        {
            $action = '';
...
if (GETPOST("$typeprod") == '0' || ($object->type == Product::TYPE_PRODUCT))
{
...
if (GETPOST("$typeprod") == '2' || ($object->type == Product::TYPE_TERRAIN))
{
...
else
{
    ...
    if ($action == 'create' && $usercancreate)
    {
        ...
        if($typeprod==1){
            ...
            //attribut fixe dans service

            /*print '<input type="hidden" name="nature_lot" id="nature_lot" value=10>';
            print '<input type="hidden" name="type" id="type" value=5>';
            print '<input type="hidden" name="statut_lot" id="statut_lot" value=1>';
            print '<input type="hidden" name="categorie" id="categorie" value=24>';*/
            ...
            if (empty($reshook))
            {
               // print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'prix_gestion');
                print '<tr><td class="fieldrequired">'.$langs->trans("Montant du service HT").'</td><td colspan="3"><input name="price" type="text" class="minwidth300 maxwidth400onsmartphone" value="'.dol_escape_htmltag(GETPOST('price','double')).'"></td></tr>';
            ...
            /*    print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'ville');
                ...
                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'nature_lot');
                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'type');
                 print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'prix_commercial');
                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'statut_lot');

                //price

                print '<tr><td class="fieldrequired">'.$langs->trans("Prix HT (demandé)").'</td><td colspan="3"><input name="price" type="text" class="minwidth50 maxwidth100onsmartphone" value="'.dol_escape_htmltag(GETPOST('price','double')).'">XOF</td></tr>';
  ...
  // print '<tr><td class="fieldrequired">'.$langs->trans("Label").'</td><td colspan="3"><input name="label" class="minwidth300 maxwidth400onsmartphone" maxlength="255" value="'.dol_escape_htmltag(GETPOST('label', 'alphanohtml')).'"></td></tr>';
            ...
            if ($typeprod != 1)	// Nature, Weight and volume only applies to products and not to services
            {
            ...
            if (empty($reshook))
            {
              ...
              /*  print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'prix_gestion');*/
                ...
                print '<tr><td class="fieldrequired">'.$langs->trans("Prix du lot HT").'</td><td colspan="3"><input name="price" type="text" class="minwidth50 maxwidth100onsmartphone" value="'.dol_escape_htmltag(GETPOST('price','double')).'">XOF</td></tr>';

                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'prix_commercial');
                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'statut_lot');
                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'position');
                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'niveau');
            }
    ...
    elseif ($object->id > 0)
    {
        ...
        if ($action == 'edit' && $usercancreate)
        {
            ...
            $typeprod = $langs->trans('Product');
            if ($object->isService()) $typeprod = $langs->trans('Service');
            //print load_fiche_titre($langs->trans('Modify').' '.$typeprod.' : '.(is_object($object->oldcopy)?$object->oldcopy->ref:$object->ref), "");
            ...
            if (empty($reshook))
            {
                ...
                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'titrefoncier');
               // print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'prix_gestion');
                print '<tr><td class="fieldrequired">'.$langs->trans("Prix du lot HT").'</td><td colspan="3"><input name="price" type="text" class="minwidth300 maxwidth400onsmartphone" value="'.dol_escape_htmltag(GETPOST('price','double')).'"></td></tr>';
                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'prix_commercial');
                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'statut_lot');
                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'position');
                print $object->showOptionalsNew($extrafields, 'edit', $parameters,'','',0,'niveau');
        ...
        else
        {
            ...
            $head = product_prepare_head($object);
            $titre = $langs->trans("CardProduct".$object->type);
            if ($object->type == Product::TYPE_SERVICE ) $picto = 'service' ;
            else if($object->type == Product::TYPE_TERRAIN || $object->type == Product::TYPE_TERRAIN ) $picto = 'product';
            ...
            if (!empty($conf->product->enabled) && !empty($conf->service->enabled))
            {
                ...
                $typeformat = 'select;0:'.$langs->trans("Product").',1:'.$langs->trans("Service").',2:'.$langs->trans("Terrain");
            ...
            // price
            print '<tr><td class="tdtop">'.$langs->trans("Prix du lot (HT)").'</td><td colspan="2">'.(dol_textishtml($object->price) ? $object->price : dol_nl2br($object->price, 1, true)).'</td></tr>';
...
if ($action != 'create' && $action != 'edit')
{
    ...
    if (empty($reshook))
    {
        if ($usercancreate)
        {
            $sql = "SELECT pe.statut_lot,pe.fk_object";
            ...
            if ($result) {
                ...
                if( $objet->statut_lot==1){
                    print '<a class="butAction" href="https://sunuerp.numherit-labs.com/comm/propal/card.php?idlot='.$object->id.'&action=create">'.$langs->trans("creerficsouscription").'</a>';
                }


            }

            /*  print '<a class="butAction" href="https://sunuerp.numherit-labs.com/comm/ficsous.php?id='.$object->id.'&save_lastsearch_values=1">'.$langs->trans("creerficsouscription").'</a>';*/
