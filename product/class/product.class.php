...
class Product extends CommonObject
{
    ...
    */
     ...
     * @return string                                String with URL
     */
    public function getNomUrlTerrain($withpicto = 0, $option = '', $maxlength = 0, $save_lastsearch_value = -1, $notooltip = 0)
    {
        global $conf, $langs, $hookmanager;
        include_once DOL_DOCUMENT_ROOT.'/core/lib/product.lib.php';
        ...
        if ($option == 'supplier' || $option == 'category') {
            ...
            $url = DOL_URL_ROOT.'/product/composition/card_terrain.php?id='.$this->id;
        } else {
            $url = DOL_URL_ROOT.'/product/card_terrain.php?id='.$this->id;
        }

        if ($option !== 'nolink') {
            // Add param to save lastsearch_values or not
            $add_save_lastsearch_values=($save_lastsearch_value == 1 ? 1 : 0);
            if ($save_lastsearch_value == -1 && preg_match('/list_terrain\.php/', $_SERVER["PHP_SELF"])) { $add_save_lastsearch_values=1;
            }
            if ($add_save_lastsearch_values) { $url.='&save_lastsearch_values=1';
            }
        }
    ...
    public function LibStatut($status, $mode = 0, $type = 0)
    {
       ...
       // return $status;
        ...
        elseif($status == 2){
            // $type   0=Status "to sell", 1=Status "to buy", 2=Status "to Batch"
            if($type==0){
                $labelstatut = $langs->trans('ReserveSell');
                $labelstatutShort = $langs->trans('ReserveSell');
                $statuttrans='status7';
            }
            ...
            // $type   0=Status "to sell", 1=Status "to buy", 2=Status "to Batch"
            if($type==0){
                $labelstatut = $langs->trans('VenduSell');
                $labelstatutShort = $langs->trans('VenduSell');
                $statuttrans='status8';
            }
