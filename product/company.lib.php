 ...
 */
function societe_prepare_head3($object)
{ 
	...
	// Ajout de l'onglet savenir si il s'agit d'un client savenir
		require_once('connexion.php'); 
 		require_once('con_gescom.php'); 
		$sql2="select email from llx_societe where rowid=".$object->id;
 
		@mysql_select_db($database_con_gescom, $con_gescom);
		$rq_sql2 = mysql_query($sql2, $con_gescom) or die(mysql_error());

		$row_rq_sql2 = mysql_fetch_assoc($rq_sql2);
		$totalRows_rq_sql2 = mysql_num_rows($rq_sql2);
		if($totalRows_rq_sql2>0){
			// Veification si c'est un client Savenir
			$sql3  = "SELECT * from compte where email like '".$row_rq_sql2['email']."'";
			mysql_select_db($database_connexion, $connexion);
			$rq_sql3 = mysql_query($sql3, $connexion) or die(mysql_error());
			$row_rq_sql3 = mysql_fetch_assoc($rq_sql3);
			$totalRows_rq_sql3 = mysql_num_rows($rq_sql3);
			// FIN verfication
			if ( ($totalRows_rq_sql3>0) )
			{
				$head[$h][0] = DOL_URL_ROOT.'/comm/prospect/savenir.php?socid='.$object->id;
				//pour colline
				if($row_rq_sql3['progamme']==8)
				{
				$head[$h][0] = DOL_URL_ROOT.'/comm/prospect/autre_prog.php?socid='.$object->id;
				 $head[$h][1] = "Colline";
				$head[$h][2] = 'Colline';
				}
				//Onglet Jade
				elseif($row_rq_sql3['progamme']==6)
				{
				$head[$h][0] = DOL_URL_ROOT.'/comm/prospect/autre_prog.php?socid='.$object->id;
				 $head[$h][1] = "Jade";
				$head[$h][2] = 'Jade';
				...
				//Les Coquillages
				elseif($row_rq_sql3['progamme']==3)
				{
				$head[$h][0] = DOL_URL_ROOT.'/comm/prospect/autre_prog.php?socid='.$object->id;
				 $head[$h][1] = "Les Coquillages";
				$head[$h][2] = 'Les Coquillages';
				}
				//Les roche
				elseif($row_rq_sql3['progamme']==1)
				{
				$head[$h][0] = DOL_URL_ROOT.'/comm/prospect/autre_prog.php?socid='.$object->id;
				 $head[$h][1] = "La Roche";
				$head[$h][2] = 'La Roche';
 ...
 * @return
 */
function societe_prepare_head4($object)
{
    global $langs, $conf, $user;
    $h = 0;
    $head = array();
 ...
 *  @return     array           head
 */
function societe_admin_prepare_head5($object)
{
    global $langs, $conf, $user;
