...
require_once(DOL_DOCUMENT_ROOT."/product/product.lib.php");
...
if ($action == 'create' && ($user->rights->produit->creer || $user->rights->service->creer))
{
	...
	if (empty($usecanvas))
	{
		...
		$statutarray=array('1' => $langs->trans("OnSell"), '0' => $langs->trans("NotOnSell"), '2' => "Réservé");
		...
		if ($_GET["type"] != 1)	// Le poids et le volume ne concerne que les produits et pas les services
		{
			...
			Select_Position('volume',$_POST["volume"]);
			print '</td></tr>';
		}
...
else if ($id || $ref)
{
	...
	if ($action == 'edit' && ($user->rights->produit->creer || $user->rights->service->creer))
	{
		if (empty($usecanvas))
		{
			...
			if ($product->status)
			{
				...
				print '>'.$langs->trans("OnSell").'</option>';
				print '<option value="0"';
				if($product->status==0)
				print ' selected="selected"';
				print '>'.$langs->trans("NotOnSell").'</option>';
				print '<option value="2"';
				if($product->status==2)
				print ' selected="selected"';
				print '>Réservé</option>';
				print '<option value="3"';
				if($product->status==3)
				print ' selected="selected"';
				print '>Vendu</option>';
				//print '<option value="4">Réservé</option>';
			}
			else
			{
				print '<option value="1">'.$langs->trans("OnSell").'</option>';
				print '<option value="0" selected="selected">'.$langs->trans("NotOnSell").'</option>';
				print '<option value="2">Réservé</option>';
				print '<option value="3">Vendu</option>';
			}
			print '</select>';
			print '</td></tr>';
			...
			// Weight
				print '<tr><td>Type</td><td colspan="2">';
				...
				Select_TypeProduct('weight',$product->weight);
				print '</td></tr>';
				// Length
				print '<tr><td>Niveau</td><td colspan="2">';
				...
				Select_NiveauProduct('size',$product->length);
				print '</td></tr>';
			...
			else
			{
				...
				print '</td></tr>';
				// Volume
				print '<tr><td>Position</td><td colspan="2">';
				...
				Select_Position('volume',$product->volume);
				print '</td></tr>';
			}
	...
	else
	{
		...
		$titre=$langs->trans("terrain test".$product->type);
		...
		if (empty($usecanvas))
		{
			...
			// Weight
			print '<tr><td>Type de Produit</td><td colspan="2">';
			if ($product->weight != '')
			{
					//print $product->weight." ".measuring_units_string($product->weight_units,"weight");
					print get_TypeProduct($product->weight);
			}
			else
			{
					print '&nbsp;';
			...
			if ($product->length != '')
			{
					...
					print get_NiveauProduct($product->length);
			}
			else
			{
					print '&nbsp;';
			...
			else
			{
				...
				if ($product->volume != '')
				{
					...
					print get_Position($product->volume);
				}
				else
				{
					print '&nbsp;';
