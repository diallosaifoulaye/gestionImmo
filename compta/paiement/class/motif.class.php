...
class Motif extends CommonObject
{
	...
	public function fetch($id, $ref = '', $fk_bank = '')
	{
		...
		$sql.= ' FROM '.MAIN_DB_PREFIX.'paiement as p LEFT JOIN '.MAIN_DB_PREFIX.'motif as m ON m.rowid=p.fk_motif';
...
var_dump($sql);die();
		...
		if ($resql)
		{
			if ($this->db->num_rows($resql))
			{
				...
				$this->descritption   = $obj->description;
				$this->code      = $obj->code;
    ...
    public function create($user, $closepaidinvoices = 0, $thirdparty = null)
	{
		...
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."motif (rowid, label, code, description, statut)";
		$sql.= " VALUES ('".$total.", ".$mtotal.", ".$this->paiementid.", '".$this->db->escape($this->num_paiement)."', '".$this->db->escape($note)."', ".($this->ext_payment_id?"'".$this->db->escape($this->ext_payment_id)."'":"null").", ".($this->ext_payment_site?"'".$this->db->escape($this->ext_payment_site)."'":"null").", ".$user->id.")";
