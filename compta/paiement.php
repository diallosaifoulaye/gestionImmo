...
if ($action == 'create' || $action == 'confirm_paiement' || $action == 'add_paiement')
{
	...
	if ($result >= 0)
	{
		...
		if ($action == 'add_paiement')
		{
			...
			$formquestion[$i++]=array('type' => 'hidden','name' => 'fk_propal', 'value' =>$idprop);
			$formquestion[$i++]=array('type' => 'hidden','name' => 'type',  'value' => $facture->type);$formquestion[$i++]=array('type' => 'hidden','name' => 'type',  'value' => $facture->type);
		}
		...
		print '<form id="payment_form" name="add_paiement" action="'.$_SERVER["PHP_SELF"].'?idprop='.$idprop.'" method="POST">';
        ...
        print '<input type="hidden" name="fk_propal" id="fk_propal" value="'.$idprop.'">';
        ...
        // Payment motif
        print '<tr><td><span class="fieldrequired">'.$langs->trans('PaymentMotif').'</span></td><td>';
      /*  $form->select_motifs_paiements((GETPOST('paiementmotif')?GETPOST('paiementmotif'):$facture->motifid), 'paiementmotif', '', 2);;*/
        $selected	= (GETPOST('fk_motif'));
        $sql = "SELECT *";
        $sql.= " FROM ".MAIN_DB_PREFIX."motif";
        $sql.= " WHERE statut=1";
        $sql.= " ORDER BY label";
        ...
        if ($resql) {
            ...
            $i = 0;
            if ($num) {
                print '<select id="select" name="fk_motif">';
                while ($i < $num) {
                    ...
                    /* if ($showcurrency) print ' ('.$obj->rowid.')';
...
//var_dump(GETPOST('fk_motif'));
            ...
            //var_dump($idprop);die;
            print $form->formconfirm($_SERVER['PHP_SELF'].'?facid='.$facture->id.'&socid='.$facture->socid.'&type='.$facture->type.'&fk_propal='.$idprop, $langs->trans('ReceivedCustomersPayments'), $text, 'confirm_paiement', $formquestion, $preselectedchoice);
        }
