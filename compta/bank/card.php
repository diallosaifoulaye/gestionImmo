...
function select_numcompte($selected='',$htmlname='numcompte',$empty=0, $htmloption='', $condition = null)
{
    global $conf,$langs, $db;
    ...
    $sql = "SELECT c.numero , c.label";
    $sql .= " FROM " . MAIN_DB_PREFIX . "compte as c ";
    $sql .= " WHERE c.entite_rowid  = " . $conf->entity;
    if(!is_null($condition)) $sql .= " AND ($condition)";
    ...
    if ($resql)
    {
        $out.= '<select id="select'.$htmlid.'" class="flat selectmode" name="'.$htmlname.'" '.$htmloption.'" >';
        $num = $db->num_rows($resql);
        $i = 0;
        if ($num)
        {
    ...
    include_once DOL_DOCUMENT_ROOT . '/core/lib/ajax.lib.php';
    $out .= ajax_combobox('select' . $htmlid);
    return $out;
}
...
if ($action == 'create')
{
    ...
    $sql = 'SELECT j.code, j.label, j.numcompte FROM ' . MAIN_DB_PREFIX . 'journal AS j INNER JOIN ' . MAIN_DB_PREFIX . 'type_journal AS tp ON j.typeJournal = tp.rowid WHERE (tp.rowid = 3 OR tp.rowid = 4) AND j.code NOT IN (SELECT ref FROM ' . MAIN_DB_PREFIX . 'bank_account) GROUP BY j.code ORDER BY tp.rowid, j.code ASC ';
    $codeJB = $db->query($sql)->fetch_all();
    ...
    <?php
	//else print '<td><input size="8" type="text" class="flat" name="ref" value="'.dol_escape_htmltag(GETPOST("ref") ?GETPOST("ref", 'alpha') : $object->ref).'" maxlength="12" autofocus></td></tr>';

	// Label
	print '<tr><td class="fieldrequired">'.$langs->trans("LabelBankCashAccount").'</td>';
	print '<td><input type="text" class="flat maxwidth150onsmartphone" style="width: 40%;" id="label-create" name="label" value="'.dol_escape_htmltag($codeJB[0][1]).'"></td></tr>';
	...
	if (!empty($conf->accounting->enabled))
	{
		...
		print '<tr><td class="'.$fieldrequired.'titlefieldcreate">'.$langs->trans("AccountancyCode").'</td><td>';
        print select_numcompte($selected=$object->account_number,$htmlname='account_number',$empty=0, $htmloption='', "c.numero LIKE '5%'");
          print '</td></tr>';
...
else
{
	...
	if (GETPOST('id', 'int') && $action == 'edit' && $user->rights->banque->configurer)
	{
        ...
        $sql = 'SELECT j.code, j.label FROM ' . MAIN_DB_PREFIX . 'journal AS j INNER JOIN ' . MAIN_DB_PREFIX . 'type_journal AS tp ON j.typeJournal = tp.rowid WHERE (tp.rowid = 3 OR tp.rowid = 4) AND j.code NOT IN (SELECT ref FROM ' . MAIN_DB_PREFIX . 'bank_account) GROUP BY j.code ORDER BY tp.rowid, j.code ASC ';
        $codeJB = $db->query($sql)->fetch_all();
        $ref = dol_escape_htmltag(GETPOST("ref") ?GETPOST("ref", 'alpha') : $object->ref);
        array_push($codeJB, [$ref]);
		...
		print '<tr><td class="fieldrequired titlefieldcreate">'.$langs->trans("Ref").'</td>'; ?>
            <td>
                <select name="ref">
                    <?php foreach ($codeJB as $item) { ?>
                        <option <?= ($ref == $item[0] ? 'selected' : '') ?> value="<?= $item[0] ?>"><?= $item[0] ?></option>
                    <?php } ?>
                </select>
            </td>
            </tr>
...
//        print '<td><input type="text" class="flat maxwidth200" name="ref" value="'.dol_escape_htmltag(isset($_POST["ref"]) ?GETPOST("ref") : $object->ref).'"></td></tr>';
		...
		if (!empty($conf->accounting->enabled)) {
            ...
            print select_numcompte($selected=$object->account_number,$htmlname='account_number',$empty=0, $htmloption='');
