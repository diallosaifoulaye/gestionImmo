...
function PLines2(&$inc, $parent, &$lines, &$level, $var, $showproject, &$taskrole, $projectsListId='')
{
	global $user, $bc, $langs;
	...
	for ($i = 0 ; $i < $numlines ; $i++)
	{
		...
		if ($lines[$i]->fk_parent == $parent)
		{
			...
			if ($showline)
			{
				...
				print "<td>";
				print '<a href="#">';
				print ListeRessourcesTaches($lines[$i]->id);
				print '</a>';
				print "</td>\n";
...
function PLines3(&$inc, $parent, &$lines, &$level, $var, $showproject, &$taskrole, $projectsListId='')
{
	...
	for ($i = 0 ; $i < $numlines ; $i++)
	{
		...
		if ($lines[$i]->fk_parent == $parent)
		{
			...
			if ($showline)
			{
				...
				print '<td align="right">';
				print '<a href="#">';
				print dol_print_date(date($lines[$i]->date_start),'day');
				print '</a>';
				print "</td>\n";
				...
				$level++;
				if ($lines[$i]->id) PLines3($inc, $lines[$i]->id, $lines, $level, $var, $showproject, $taskrole, $projectsListId);
				$level--;
				$total += $lines[$i]->duration;
			}
		}
...
function print_projecttasks_array2($db, $socid, $projectsListId, $mytasks=0)
{
	global $langs,$conf,$user,$bc;
	...
	print '<tr class="liste_titre">';
	print_liste_field_titre($langs->trans("Project"),"index.php","","","","",$sortfield,$sortorder);
	print_liste_field_titre("Progression","","","","",'align="right"',$sortfield,$sortorder);
	print_liste_field_titre($langs->trans("NbOpenTasks"),"","","","",'align="right"',$sortfield,$sortorder);
	print_liste_field_titre($langs->trans("Status"),"","","","",'align="right"',$sortfield,$sortorder);
	print "</tr>\n";

	$sql="SELECT p.rowid as projectid, p.ref, p.title, p.fk_user_creat, p.public, p.fk_statut, COUNT(t.rowid) as nb, SUM(t.progress) as somme ";
	$sql.= " FROM ".MAIN_DB_PREFIX."projet as p";
	if ($mytasks)
	{
        $sql.= ", ".MAIN_DB_PREFIX."projet_task as t";
	...
	if ( $resql )
	{
		...
		while ($i < $num)
		{
			...
			if ($userAccess >= 0)
			{
				$var=!$var;
				print "<tr ".$bc[$var].">";
				print '<td nowrap="nowrap"><a href="http://www.sabluxgroup.com/gescom/htdocs/projet/fiche.php?id='.$objp->projectid.'">';
				print ''.$objp->title.'</a></td>';
				if($objp->nb)
				$var=($objp->somme/(100*$objp->nb));
				else 
				$var=0;
				print '<td align="right">'.intval(($var*100)).'%</td>';
				print '<td align="right">'.$objp->nb.'</td>';
				$projectstatic->statut = $objp->fk_statut;
				print '<td align="right">'.$projectstatic->getLibStatut(3).'</td>';
				print "</tr>\n";
...
function ListeRessourcesTaches($idtache=0)
{
	global $db,$langs,$conf,$user,$bc;
	$sql = "SELECT  ";
	$sql.= " t.civilite as civility, t.name as lastname, t.firstname ";
	$sql.= " FROM ".MAIN_DB_PREFIX."projet_task pr ,".MAIN_DB_PREFIX."c_type_contact tc ";
	$sql.= ", ".MAIN_DB_PREFIX."element_contact ec ";
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."user t on ec.fk_socpeople = t.rowid ";
	$sql.= " WHERE ec.element_id = pr.rowid ";
	$sql.= " AND ec.fk_c_type_contact=tc.rowid ";
	$sql.= " AND tc.element='project_task' ";
	$sql.= " AND tc.source = 'internal' ";
	$sql.= " AND tc.active=1 AND pr.rowid= ".$idtache;
	...
	if ( $resql )
	{
		...
		while ($i < $num)
		{
			...
			if ( $i>=1)
			$varialbe .= ', ';
			$varialbe .= $obj->firstname.' '.$obj->lastname;
			$i++;
		}
