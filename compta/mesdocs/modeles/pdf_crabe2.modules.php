...
/**
 *	\file       htdocs/includes/modules/facture/doc/pdf_crabe2.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate invoices from crab model
 *	\author	    Laurent Destailleur
 *	\version    $Id: pdf_crabe2.modules.php,v 1.12 2011/07/31 23:28:15 eldy Exp $
 */
...
/**
 *	\class      pdf_crabe2
 *	\brief      Classe permettant de generer les factures au modele crabe2
 */

class pdf_crabe2 extends ModelePDFFactures
{
	...
	function pdf_crabe2($db)
	{
		...
		$this->db = $db;
		$this->name = "crabe2";
		$this->description = $langs->trans('PDFcrabe2Description');
...
//print "$this->marge_gauche, $tab_top-1, $this->page_largeur-$this->marge_gauche-$this->marge_droite, $height_note+1";exit;
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			$pdf->SetXY ($col2x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($largcol2, $tab2_hl, price($object->total_ttc)." ".$bocar, $useborder, 'R', 1);
		}
		$pdf->SetTextColor(0,0,0);
		...
		// on change le status des produits
		if ($object->paye) 
		{	
			$nblignes = sizeof($object->lines);
			...
			for ($i = 0 ; $i < $nblignes ; $i++)
			{
				$resez = $product2->fetch($object->lines[$i]->fk_product);
				$retour="-2";
				if($product2->isproduct())
				{
							$product2->setStatus($object->lines[0]->fk_product,3);
				}
			}
		...
		{	
			...
			for ($i = 0 ; $i < $nblignes ; $i++)
			{
				...
				if($product2->isproduct())
				{
							$product2->setStatus($object->lines[0]->fk_product,2);
				}
			}
