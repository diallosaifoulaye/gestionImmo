...
if (empty($reshook))
{
	...
	elseif ($action == 'add' && $usercancreate)
	{
	   ...
	   /* var_dump(  ($_POST['type'] == 3?1:$_POST['cond_reglement_id'])." ".
        $fac->mode_reglement_id	= $_POST['mode_reglement_id']." ".
        GETPOST('fk_account', 'int')." ". $_POST['amount']." ".
      $_POST['remise_absolue']." ".$_POST['remise_percent']." ".
       GETPOST('incoterm_id', 'int')." ". GETPOST('location_incoterms', 'alpha')." ".
        GETPOST('multicurrency_code', 'alpha')." ". GETPOST('originmulticurrency_tx', 'int')." ".
        (GETPOSTISSET('entity')?GETPOST('entity', 'int'):$conf->entity)
    );die();*/
    ...
    elseif ($idech and count($object->lines)<=0 ){
	    //var_dump(count($object->lines));die();
        $object->id=$id;
        $sql = "SELECT * FROM " .MAIN_DB_PREFIX."echeance  WHERE rowid = ".$idech;
        $resql = $db->query($sql);
        if ($resql){
            $obj = $db->fetch_object($resql);
            $result = $object->addlinefact($obj->etape, $obj->montant, 1, 0, 0, 0, $idech, 0, '', '', 'TTC');
...
if ($action == 'create')
{
	...
	print '<input type="submit" class="button" name="bouton" value="' . $langs->trans('Suiivant') . '">';
	print '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
...
elseif ($id > 0 || ! empty($ref))
{
    ...
    print '<a href="' . DOL_URL_ROOT . '/compta/facture/list.php?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';

    $idprop=GETPOST('idprop','int');
    //var_dump($idprop);die;
    $linkback = '<a href="' . DOL_URL_ROOT . '/comm/propal/card.php?id='.$idprop.'&save_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("Retourfichesouscriprion") . '</a>';
	...
	$sql = 'SELECT p.datep as dp, p.ref, p.num_paiement, p.rowid, p.fk_bank,p.fk_motif,';
...
//var_dump($sql);die();
	...
	if ($result) {
		...
		if ($num > 0) {
			while ($i < $num) {
				...
				print '<tr class="oddeven"><td nowrap="nowrap">';
				print $paymentstatic->getNomUrl(1);
				print '</td>';
				print '<td nowrap="nowrap">' . dol_print_date($db->jdate($objp->dp), 'dayhour') . '</td>';
				$label = ($langs->trans("PaymentType" . $objp->payment_code) != ("PaymentType" . $objp->payment_code)) ? $langs->trans("PaymentType" . $objp->payment_code) : $objp->payment_label;
				print '<td nowrap="nowrap">' . $label . ' ' . $objp->num_paiement . '</td>';
				if (! empty($conf->banque->enabled))
				{
					...
					print '<td nowrap="nowrap" class="left">';
					if ($bankaccountstatic->id)
						print $bankaccountstatic->getNomUrl(1, 'transactions');
					print '</td>';
				}
				print '<td nowrap="nowrap" class="right">' . price($sign * $objp->amount) . '</td>';
				if ( $objp->fk_motif){
                $sql1 = 'SELECT label';
                $sql1 .= ' FROM ' . MAIN_DB_PREFIX . 'motif as m';
                $sql1 .= ' WHERE m.rowid = ' . $objp->fk_motif;
                $result1 = $db->query($sql1);
                $objmotif = $db->fetch_object($result1);

				print '<td nowrap="nowrap" class="left">' .$objmotif->label . '</td>';
                }
				else	{
    ...
    if (!empty($object->lines)) {
        ...
        $ret = $object->printObjectLines($action='setstatut', $mysoc, $soc, $lineid, 1);
		...
		if (empty($reshook)) {
			...
			if ($object->type != Facture::TYPE_CREDIT_NOTE && $object->statut == 1 && $object->paye == 0 && $usercanissuepayment) {
				if ($objectidnext) {
					...
					//} else {
						print '<div class="inline-block divButAction"><a class="butAction" href="'. DOL_URL_ROOT .'/compta/paiement.php?facid=' . $object->id . '&idech=' . $idech . '&amp;action=create&amp;accountid='.$object->fk_account.'">' . $langs->trans('DoPayment') . '</a></div>';
					//}
				}
			}
    ...
    //print "tesssssssssssssssstttttt\n";
	...
	if ($action != 'prerelance' && $action != 'presend')
	{
		...
		$urlsource = $_SERVER['PHP_SELF'] . '?facid=' . $object->id.'&ref_facture='.$object->ref;
