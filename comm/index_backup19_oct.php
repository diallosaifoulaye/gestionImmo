...
// Stat pour les tiers
    ...
    print '<table class="noborder" width="100%">';
    print '<tr class="liste_titre"><td colspan="2">Nombre de client/prospect par commercial</td>';
    print '<td>&nbsp;</td>';
    print '<td align="right">';
    print '<form action='.$_SERVER['PHP_SELF'].' method="post" name="form_mois" id="form_mois">';
    print '<select name="mois_com" id="mois_com" onchange="document.form_mois.submit();" >';
    print '<option value="">Choisir Mois</option>';
    $result1 = $db->query($sql1);

    if ($result1)
    {
        while ($objp1 = $db->fetch_object($result1))
        {
            print '<option value="'.$objp1->mois_int.';'.$objp1->annee.'"' ;
            if (!(strcmp($objp1->mois_int.';'.$objp1->annee,$ladate)))
            {print  ' selected="selected"';}
            print ' >'.$langs->trans($objp1->mois).' '.$objp1->annee;
            print '</option>';
        }
    }
    print '</select>';
...
///Affichage du resultat
    ...
    if ($result)
    {
        print '<tr class="liste_titre"><td colspan="2"><a href="#">Commercial</a></td>';
        print '<td align="right"><a href="#">Propect</a></td>';
        print '<td align="right"><a href="#">Client</a></td>';
        print '</tr>';
        ...
        while ($objp = $db->fetch_object($result))
        {
            ...
            print '<tr  '.$bc[$var].'><td colspan="2">'.$objp->firstname.' '.$objp->lastname.'</td>';
            print '<td align="right">'.$objp2->nb_pros .'</td>';
            print '<td align="right">'.$objp3->nb_client .'</td>';
            print '</tr>';
        }
    }
