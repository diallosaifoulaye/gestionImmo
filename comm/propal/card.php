...
// Security check
/*if (! empty($idlot));
$result = restrictedArea($idlot, 'propal', $id);*/
...
//var_dump(GETPOST(socid));die();
        ...
        if ($result >= 0) {
            if (empty($conf->global->MAIN_DISABLE_PDF_AUTOUPDATE)) {
              ...
              //  var_dump($socid);die();
                ...
                //géénérer  contrat
                //s $object->generateDocument("contrat", $outputlangs, $hidedetails, $hidedesc, $hideref);
                // on change le  statut  du produits
                // echo "<pre>";var_dump($this->id);die;
                $sql = 'UPDATE ' . MAIN_DB_PREFIX . 'product SET tosell = 2 WHERE rowid = ' . $object->lines[0]->fk_product;
    ...
    elseif ($action == 'add' && $usercancreate) {
        ...
        if (empty($duration)) {
            ...
            $action = 'create';
            /* $action="https://sunuerp.numherit-labs.com/comm/propal/card.php?action=create&idlot=".$idlot;
             var_dump($action);die();*/
            $error++;
        }
        ...
        if (!$error) {
            ...
            if (!$error) {
                ...
                if ($id > 0) {
                    ...
                    $sql .= " WHERE pe.fk_object = " . $idlot;
                    $resql = $db->query($sql);
                    if ($resql) {
                        $obj = $db->fetch_object($resql);
                        $result = $object->addline($desc, $obj->prix_commercial, 1, 18, 0, 0, $idlot);
                    ...
                    if (!$error) {
                        ...
                        header('Location: ' . $_SERVER["PHP_SELF"] . '?id=' . $id . '&idlot=' . $idlot);
                        exit();
                    } else {
                        $db->rollback();
                        $action = 'create';
...
{ //var_dump($_POST);die();
    ...
    /*$action="comm/propal/card.php?action=create&idlot=".$idlot;
          var_dump($action." ".$_SERVER["PHP_SELF"]);die();*/
    if (GETPOST('idlot'))
        print '<form name="addprop" action="' . $_SERVER["PHP_SELF"] . '?idlot=' . GETPOST('idlot') . '" method="POST">';
    else
   ...
   // var_dump($socid);die();
    ...
    if ($socid > 0) {
        ...
        print '<td>';
        print $form->select_company($socid, 'socid', 's.fournisseur=0 or s.fournisseur=1', 'SelectThirdParty', 0, 0, null, 0, 'minwidth300');
  ...
  /*  print '<tr>';
    ...
    print '<input type="submit" class="button" value="' . $langs->trans("Suivant") . '">';
    print '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
...
// Confirm delete
    ...
    elseif ($action == 'validate') {
        ...
        //var_dump($object->socid);die();

        $text = $langs->trans('ConfirmValidateProp') . ' ' . $numref;
       //echo '<pre>';
      //  var_dump($object->lines[0]->fk_product);die();
       $idprodchoisi=$object->lines[0]->fk_product;
        $sql = "SELECT *";
        $sql .= " FROM ".MAIN_DB_PREFIX."categorie_product as cp";
       // $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."categorie as c ON c.rowid=p.fk_soc";
        $sql .= " WHERE cp.fk_product = ".(int) $idprodchoisi;
        //var_dump($sql);die();
        $resql = $db->query($sql);
        if ($resql){
            $objcp= $db->fetch_object($resql);
            $idcatchoisit=$objcp->fk_categorie;
            $sql1 = "SELECT c.*,ec.etape,ec.fk_categorie,ec.date_deb,ec.date_fin, ec.montant,ec.statut";
            $sql1 .= " FROM ".MAIN_DB_PREFIX."categorie c";
           $sql1.= " LEFT JOIN ".MAIN_DB_PREFIX."echeance_categorie as ec ON ec.fk_categorie = c.rowid";
            $sql1 .= " WHERE ec.fk_categorie = ".(int) $idcatchoisit;
            //var_dump($sql1);die();
           ...
           if ($resql1){
            ...
            $i = 0;
            while ($i < $num) {
                $objec= $db->fetch_object($resql1);
                  $sqlm = "SELECT SUM(montant) as somech";
                    $sqlm .= " FROM ".MAIN_DB_PREFIX."echeance as e";
                    $sqlm .= " WHERE e.fk_propal = ".(int) $id;

                    $resqlm = $db->query($sqlm);
                    $objm= $db->fetch_object($resqlm);
                   // while ($objm->somech+GETPOST('montant', 'int')<=$obj->total){
                        $objecte = new echeance($db);
                        $objecte->etape = $objec->etape;
              //  var_dump($objecte);die();
                        $objecte->date_deb = $objec->date_deb;
                        $objecte->date_fin = $objec->date_fin;
                        $objecte->montant = $objec->montant;
                        $objecte->description = $objec->description;
                        $objecte->fk_propal =$id;
                        $objecte->statut = $objec->statut;
                        $result = $objecte->create($user);

                        //var_dump($result);die();


//rowid de la nouvelle échéance
                        $rowid=$objecte->getrowid();
...
//var_dump($rowid);die();
               ...
               // var_dump($objetf);die();
                        ...
                        $ref			= "ACC_";
                        $date				= date("Y-m-d");
                        $ref_client			=  $objetf->ref_client;
                        $modelpdf			= "crabe";
                        $cond_reglement_id	= 1;
                        $mode_reglement_id	= 4;
                        $fk_account         = -1;
                        $amount				= $objetf->montant;
                        $entity				= $objetf->entity;
                        $statut= 0;
               ...
               // var_dump($resql);die();
                        if($resql){
                           // var_dump(1111);die();
                            ...
                            $ref='(ACC_'.$idf.')';
                            $sql = 'UPDATE '.MAIN_DB_PREFIX."facture SET ref='".$db->escape($ref)."' WHERE rowid=".$idf;
                            $resql = $db->query($sql);
        ...
        if (!$error)
           // var_dump(1111);die();
            $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id . '&socid=' . $object->socid, $langs->trans('ValidateProp'), $text, 'confirm_validate', '', 0, 1);
        //var_dump($formconfirm);die();
    }
        ...
        if (empty($reshook)) {
            if ($action != 'editline') {
                ...
                if ($object->statut == Propal::STATUS_DRAFT && $object->total_ttc >= 0 && count($object->lines) > 0) {
                    if ($usercanvalidate) {
                        print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&socid=' . $object->socid . '&amp;action=validate">' . $langs->trans('Validate') . '</a></div>';
                    } else
                ...
                if ($conf->contrat->enabled && $object->statut == Propal::STATUS_SIGNED) {
                    ...
                    if ($usercancreatecontract) {
                        //print '<div class="inline-block divButAction"><a class="butAction" disabled href="' . DOL_URL_ROOT . '/contrat/card.php?action=create&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '" >' . $langs->trans('AddEcheance') . '</a></div>';
                        /*					print '<div class="inline-block divButAction"><a class="butAction" disabled href="' . DOL_URL_ROOT . '/categories/echeance/card.php?action=addEcheance&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '" >' . $langs->trans('AddEcheance') . '</a></div>';*/
                    }
                }
                // Create contract
                if ($object->statut == Propal::STATUS_SIGNED) {
                    $sql = "SELECT pd.rowid as pdid,pd.fk_propal,pd.fk_product,";
                    $sql.= ' pe.statut_lot';
                    $sql.= ' FROM '.MAIN_DB_PREFIX.'propaldet as pd';
                    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."product_extrafields as pe ON pe.fk_object=pd.fk_product";
                    ...
                    if ($result) {
                        ...
                        $sql.= " SET statut_lot = 4";
                        $sql.= " WHERE fk_object = " . $objet->fk_product;
                        //var_dump($sql);die();
                        $result = $db->query($sql);
...
//            }
                ...
                //if ($object->statut > 1) {
                $sql = "SELECT * FROM " . MAIN_DB_PREFIX . "echeance WHERE fk_propal = " . $id;
                dol_syslog($script_file . " sql=" . $sql, LOG_DEBUG);
                $resql = $db->query($sql);
                $num = false;
                if ($resql) {
                ...
                if($num > 0) {
                   if ($object->statut !== Propal::STATUS_SIGNED && $usercanclose) {

                        print '<div class="inline-block divButAction">
      <a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&amp;action=statut' . (empty($conf->global->MAIN_JUMP_TAG) ? '' : '#close') . '"';
                        print '>' . $langs->trans('SetAcceptedRefused') . '</a></div>';
                   }
                }

                $sql1 = "SELECT * FROM " . MAIN_DB_PREFIX . "echeanceTerrain WHERE fk_propal = " . $id;
                dol_syslog($script_file . " sql=" . $sql1, LOG_DEBUG);
                $resql1 = $db->query($sql1);
                $num1 = false;
                if ($resql1) {
       ...
       // print '<a name="builddoc"></a>'; // ancre
        ...
        if ($object->statut != 0){
           ...
           // var_dump($urlsource);die();
            ...
            print '<span style="color: rgb(110,80,20);">' . $langs->trans('listoffactures') . '</span>';
            print '</div>';
            print '<table class="noborder allwidth" data-block="showLinkedObject" data-element="propal" data-elementid="189">';
            ...
            print_liste_field_titre($langs->trans('factures'), $_SERVER['PHP_SELF'], 't.statut', '', $param, '', '', '');
            print_liste_field_titre($langs->trans('   '), $_SERVER['PHP_SELF'], 't.date', '', $param, '', '', '');
            print_liste_field_titre($langs->trans('Statut'), $_SERVER['PHP_SELF'], 't.date', '', $param, '', '', '');

            /* print_liste_field_titre($langs->trans('programmes'), $_SERVER['PHP_SELF'], 't.programme', '', $param, '', '', '');*/
            print '</tr>';

            $sqlf = 'SELECT f.*,';
            $sqlf .= ' e.fk_facture,e.fk_propal';
            $sqlf .= ' FROM ' . MAIN_DB_PREFIX . 'echeance as e';
            $sqlf .= ' LEFT JOIN ' . MAIN_DB_PREFIX . 'facture as f ON f.rowid = e.fk_facture' ;
            $sqlf .= " WHERE e.fk_propal = " . $id;
            // var_dump($sqlf);die();
            $resqlf = $db->query($sqlf);
            if ($resqlf) {
                $num = $db->num_rows($resqlf);
                $i = 0;
                if ($num){
                    while ($i < $num) {
                        $obj2 = $db->fetch_object($resqlf);
                        $var = !$var;
                        if ($obj2) {
                            print "<tr " . $bc[$var] . ">";
                            print '<td>';
                          print  '<a class="documentdownload paddingright" href="/document.php?modulepart=facture&amp;file='.$obj2->ref.'/'.$obj2->ref.'.pdf&amp;entity=1"><i class="fa fa-file-pdf-o paddingright" title="Fichier: '.$obj2->ref.'.pdf"></i>'.$obj2->ref.'.pdf</a>';
                            print '</td>';
                            print '<td>';
...
//$somethingshown = $formactions->showactions($object, 'propal', $socid, 1);
            ...
            $sqle = "SELECT DISTINCT e.*";
            $sqle .= " FROM " . MAIN_DB_PREFIX . "echeance as e";
            $sqle .= " WHERE e.fk_propal = " . $id;
            // var_dump($sqle);die();
     ...
     $sql = "SELECT *";
       ...
       // $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."categorie as c ON c.rowid=p.fk_soc";
        ...
        if ($resql) {
            ...
            $sql1 = "SELECT c.type";
            $sql1 .= " FROM ".MAIN_DB_PREFIX."categorie c";
            $sql1 .= " WHERE c.rowid = ".(int) $idcatchoisit;
           // var_dump($sql1);die();
            $resql1 = $db->query($sql1);
              if ($resql1) {
            $objct = $db->fetch_object($resql1);

        }
        }
   // var_dump($objct->type);die();

    if ($objct->type==0){

        // List of echeance maison
        print '<br>';
        print '<div style="margin-bottom: 10px;">';
        print '<span style="color: rgb(110,80,20);">' . $langs->trans('listofecheances') . '</span>';
        print '<span style="right: 20px;position: absolute;"><a class="butActionNew" href="' . DOL_URL_ROOT . '/categories/echeance/card.php?action=addEcheance&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '"><span class="text-plus-circle">' . $langs->trans('AddEcheance') . '</span><span class="fa fa-plus-circle valignmiddle"></span></a></span>';
        print '</div>';
        print '<table class="noborder allwidth" data-block="showLinkedObject" data-element="propal" data-elementid="189">';
        ...
        print_liste_field_titre($langs->trans('factures'), $_SERVER['PHP_SELF'], 't.facture', '', $param, '', '', '');
        print_liste_field_titre($langs->trans('actions'), $_SERVER['PHP_SELF'], 't.action', '', $param, '', '', '');
        ...
        if ($resqle) {
            ...
            if ($num) {
                while ($i < $num) {
                    $obje = $db->fetch_object($resqle);
                    //Recuperation de l'id facture dans echance
                    $id_fk_factures = "SELECT  e.fk_facture";
                    $id_fk_factures .= " FROM " . MAIN_DB_PREFIX . "echeance as e";
                    $id_fk_factures .= " WHERE e.rowid = " . $obje->rowid;
                    // var_dump($sqle);die();
                    $obj_fk_facture = $db->query($id_fk_factures);
                    $id_fk_facture = $db->fetch_object($obj_fk_facture);
                    $fac_id = $id_fk_facture->fk_facture;

                    //Recuperation de fk_facture dans paiement_facture
                    $facture_id = "SELECT  pf.fk_facture";
                    $facture_id .= " FROM " . MAIN_DB_PREFIX . "paiement_facture as pf";
                    $facture_id .= " WHERE pf.fk_facture = " . $fac_id;

                    $obj_facture_id = $db->query($facture_id);
                    $obj_factures_id = $db->fetch_object($obj_facture_id);
                    $obj_fac_id = $obj_factures_id->fk_facture;
                   // var_dump($obj_fac_id);die();
                    $var = !$var;
                    if ($obje) {
// You can use here results
                        ...
                        print '</td>';
                        print '<td nowrap="nowrap">';
                        print dol_print_date($db->jdate($obje->date_deb), 'day');
                        print '</td>';
                        print '<td nowrap="nowrap">';
                        print dol_print_date($db->jdate($obje->date_fin), 'day');
                        print '</td>';
                        print '<td align="right"  nowrap="nowrap">' . number_format($obje->montant, 0, ',', ' ') . '</td>';
                        ...
                        if ($object->statut == Propal::STATUS_SIGNED) {
                            print '<td align="center" style="color: darkred">';
                            print '<a href="/compta/facture/card.php?facid='.$obje->fk_facture.'&idech='.$obje->rowid.'&idprop='.$id.'" title="facture"><i class="fas fa-file" style="margin-right:8px;"></i></a>';
                            print '</td>';
                        }else{
                            print '<td align="center" style="color: red" nowrap="nowrap">';
                            print "Acceptez la souscription";
                            print '</td>';
                        }

                        if(!$obj_fac_id){
                            print '<td  align="center">';
                            print '<a href="/categories/echeance/card.php?leftmenu=categorie&action=updateEcheance&rowid='.$obje->rowid.'&amp;originid=' . $object->id.'" title="Modifier"><i class="fas fa-edit" style="margin-right:8px;"></i></a>';
                            ?>
<!--                            <a  href="/categories/echeance/card.php?leftmenu=categorie&action=delete&rowid=<?/*= $obje->rowid */?>&originid=<?/*=  $object->id */?>" onclick="return confirm('Etes vous sur de vouloir supprimer cette échéance ?');" title="Supprimer"><i class="fas fa-trash"></i></a>
-->                            <?php
                            print '</td>';
                        }
    ...
    else if ($objct->type==1){
        $sqlet = "SELECT DISTINCT etd.*,et.duree,et.date_deb,et.statut,et.montant,et.periodicite";
        $sqlet .= " FROM " . MAIN_DB_PREFIX . "echeanceTerraindet as etd";
        $sqlet.= " LEFT JOIN ".MAIN_DB_PREFIX."echeanceTerrain as et ON et.rowid=etd.fk_echeanceTerrain";
        $sqlet .= " WHERE et.fk_propal = " . $id;
        $resqlet = $db->query($sqlet);
       // var_dump($resqlet);die();
        if ($resqlet) {
            $numet = $db->num_rows($resqlet);
           // var_dump($numet);die();
            if ($numet<=0) {

                // List of echeanceterrain
                print '<br>';
                print '<div style="margin-bottom: 10px;">';
                print '<span style="color: rgb(110,80,20);">' . $langs->trans('listofecheances') . '</span>';
                print '<span style="right: 20px;position: absolute;"><a class="butActionNew" href="' . DOL_URL_ROOT . '/categories/echeanceTerrain/card.php?action=addEcheance&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '"><span class="text-plus-circle">' . $langs->trans('AddEcheance') . '</span><span class="fa fa-plus-circle valignmiddle"></span></a></span>';
                print '</div>';
                ...
                /*    print '<tr><td colspan="9" align="right"><a class="butAction" href="card.php?id=all&leftmenu=categorie&action=addEcheance&typeprod=0" title="Ajouter ">Ajouter </a></td></tr>';*/
                print '<tr class="liste_titre">';
                print_liste_field_titre($langs->trans('Num Echeance'), $_SERVER['PHP_SELF'], 't.periodicite', '', $param, '', '', '');
                print_liste_field_titre($langs->trans('date_deb'), $_SERVER['PHP_SELF'], 't.date_deb', '', $param, '', '', '');
                print_liste_field_titre($langs->trans('montant'), $_SERVER['PHP_SELF'], 't.montant', '', $param, '', '', '');
                print_liste_field_titre($langs->trans('statut '), $_SERVER['PHP_SELF'], 't.statut', '', $param, '', '', '');
                print_liste_field_titre($langs->trans('factures'), $_SERVER['PHP_SELF'], 't.facture', '', $param, '', '', '');
/*                print_liste_field_titre($langs->trans('actions'), $_SERVER['PHP_SELF'], 't.action', '', $param, '', '', '');*/
                ...
                print '<tr>';
                            ...
                            if ($numet) {
                                while ($i < $numet) {
                                    $objetd = $db->fetch_object($resqlet);
                                   // var_dump($objetd);die();
                                    if ($objetd) {
// You can use here results
                                        $num=$i+1;
                                        ...
                                        /*if($i=0){*/
                                            print '<td nowrap="nowrap">';
                                            print dol_print_date($db->jdate($objetd->date_deb), 'day');
                                            print '</td>';
                                      /*  }else   if($i>0){
                                            $dateduree=$objetd->date_deb+$objetd->duree;
                                            print '<td nowrap="nowrap">';
                                            print dol_print_date($db->jdate($dateduree), 'day');
                                            print '</td>';

                                        }*/

                                        print '<td align="right"  nowrap="nowrap">' . number_format($objetd->montant, 0, ',', ' ') . '</td>';
                                        ...
                                        if ($object->statut == Propal::STATUS_SIGNED) {
                                            print '<td align="center" style="color: darkred">';
                                            print '<a href="/compta/facture/card.php?facid='.$objetd->fk_facture.'&idech='.$objetd->rowid.'&idprop='.$id.'" title="facture"><i class="fas fa-file" style="margin-right:8px;"></i></a>';
                                            print '</td>';
                                        }else{
                                            print '<td align="center" style="color: red" nowrap="nowrap">';
                                            print "Acceptez la souscription";
...
/*                                            print '</td>';
               ...
               // var_dump($sqlet);die();
                           //$objet = $db->fetch_object($resqlet);
                         //  echo '<pre>';
                          // var_dump($objet);die();
                /*           //  var_dump($objet);die();
                            //Recuperation de l'id facture dans echance
                            $id_fk_facturet = "SELECT  et.fk_facture";
                            $id_fk_facturet .= " FROM " . MAIN_DB_PREFIX . "echeanceTerrain as et";
                            $id_fk_facturet .= " WHERE et.rowid = " . $objet->rowid;
                           //  var_dump($id_fk_facturet);die();
                            $obj_fk_facturet = $db->query($id_fk_facturet);
                            $id_fk_facturet = $db->fetch_object($obj_fk_facturet);
                            $fac_idt = $id_fk_facturet->fk_facture;

                            //Recuperation de fk_facture dans paiement_facture
                            $facture_id = "SELECT  pf.fk_facture";
                            $facture_id .= " FROM " . MAIN_DB_PREFIX . "paiement_facture as pf";
                            $facture_id .= " WHERE pf.fk_facture = " . $fac_idt;
                            $obj_facture_id = $db->query($facture_id);
                            $obj_factures_id = $db->fetch_object($obj_facture_id);
                            $obj_fac_id = $obj_factures_id->fk_facture;
                            $var = !$var;
        ...
        // You can use here results
                                print "<tr " . $bc[$var] . ">";
                                print '<td nowrap="nowrap">';
                                print   $objet->periodicite;
                                print '</td>';
                                print '<td nowrap="nowrap">';
                                print   $objet->duree;
                                print '</td>';
                                print '<td nowrap="nowrap">';
                                print dol_print_date($db->jdate($objet->date_deb), 'day');
                                print '</td>';
                                print '<td nowrap="nowrap">';
                                print dol_print_date($db->jdate($objet->montant), 'day');
                                print '</td>';
                                print '<td align="right"  nowrap="nowrap">' . number_format($objet->montant, 0, ',', ' ') . '</td>';
                                ...
                                if ($object->statut == Propal::STATUS_SIGNED) {
                                    print '<td align="center" style="color: darkred">';
                                    print '<a href="/compta/facture/card.php?facid='.$objet->fk_facture.'&idech='.$objet->rowid.'" title="facture"><i class="fas fa-file" style="margin-right:8px;"></i></a>';
                                    print '</td>';
                                }else{
                                    print '<td align="center" style="color: red" nowrap="nowrap">';
                                    print "Acceptez la souscription";
                                ...
                                if(!$obj_fac_id){
                                    print '<td  align="center">';
                                    print '<a href="/categories/echeance/card.php?leftmenu=categorie&action=updateEcheance&rowid='.$objet->rowid.'&amp;originid=' . $object->id.'" title="Modifier"><i class="fas fa-edit" style="margin-right:8px;"></i></a>';
                                    */?><!--
                                    <a  href="/categories/echeance/card.php?leftmenu=categorie&action=delete&rowid=<?/*= $objet->rowid */?>&originid=<?/*=  $object->id */?>" onclick="return confirm('Etes vous sur de vouloir supprimer cette échéance ?');" title="Supprimer"><i class="fas fa-trash"></i></a>
                                    --><?php
/*                                    print '</td>';
                                }
...
// reglement tableau
            ...
            $totalpropal= $object->total;

            print '<br>';
            print '<div style="margin-bottom: 10px;">';
            print '<span style="color: rgb(110,80,20);">' . $langs->trans('Réglements du client') . '</span>';
            //print '<span style="right: 20px;position: absolute;"><a class="butActionNew" href="' . DOL_URL_ROOT . '/categories/echeance/card.php?action=addEcheance&amp;origin=' . $object->element . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '"><span class="text-plus-circle">' . $langs->trans('AddEcheance') . '</span><span class="fa fa-plus-circle valignmiddle"></span></a></span>';
            print '</div>';
            print '<table class="noborder allwidth" data-block="showLinkedObject" data-element="propal" data-elementid="189">';
            ...
            if (! empty($conf->banque->enabled)) {
                print '<td class="liste_titre left">' . $langs->trans('BankAccount') . '</td>';
            }
            print '<td class="liste_titre right">' . $langs->trans('Amount') . '</td>';
         /*   print '<td class="liste_titre right">' . $langs->trans('Motif') . '</td>';*/
            print '<td class="liste_titre" width="18">&nbsp;</td>';
            print '</tr>';

            $sqlr = 'SELECT pf.fk_paiement,';
            $sqlr .= ' p.ref,pf.amount,p.datep,';
          /*  $sqlr .= ' m.label,';*/
            $sqlr .= ' b.banque,';
            $sqlr .= ' e.fk_facture,e.fk_propal,';
            $sqlr .= ' pd.fk_product';
            $sqlr .= ' FROM ' . MAIN_DB_PREFIX . 'echeance as e';
            $sqlr .= ' LEFT JOIN ' . MAIN_DB_PREFIX . 'facture as f ON f.rowid = e.fk_facture' ;
            $sqlr .= ' LEFT JOIN ' . MAIN_DB_PREFIX . 'paiement_facture as pf ON pf.fk_facture = e.fk_facture' ;
            $sqlr .= ' LEFT JOIN ' . MAIN_DB_PREFIX . 'paiement as p ON p.rowid = pf.fk_paiement' ;
          /*  $sqlr .= ' LEFT JOIN ' . MAIN_DB_PREFIX . 'motif as m ON m.rowid = p.fk_motif' ;*/
            $sqlr .= ' LEFT JOIN ' . MAIN_DB_PREFIX . 'bank as b ON b.rowid = p.fk_bank' ;
            $sqlr .= ' LEFT JOIN ' . MAIN_DB_PREFIX . 'propaldet as pd ON pd.fk_propal = e.fk_propal' ;
            $sqlr .= " WHERE e.fk_propal = " . $id;
            $sqlr .= " AND pf.amount > 0";

            $resqlr = $db->query($sqlr);

            if ($resqlr) {

                $num = $db->num_rows($resqlr);
                $i = 0;
                if ($num > 0){
                    while ($i < $num) {
                        $obj2= $db->fetch_object($resqlr);
                        $var = !$var;
                        if ($obj2) {
                            $sql = "UPDATE ".MAIN_DB_PREFIX."product_extrafields";
                            $sql.= " SET statut_lot = 6";
                            $sql.= " WHERE fk_object = " . $obj2->fk_product;
                            $result = $db->query($sql);
                            print "<tr " . $bc[$var] . ">";
                            print '<td>';
                            print   $obj2->ref;
                            print '</td>';
                            print '<td>';
                            print dol_print_date($db->jdate($obj2->datep), 'day');
                            print '</td>';
                            if ($obj2->banque) {
                                print '<td>';
                                print   $obj2->banque;
                            ...
                            print  number_format($obj2->amount, 0, ',', ' ');
                            print '</td>';
              ...
              /*  else {
                ...
                print '<tr class="liste_titre">';
                    ...
                    print '<td> </td>';
                    print '<td> </td>';
                    print '<td align="right" class="liste_titre">'.number_format($total, 0, ',', ' ').'</td>';
                    print '<td> </td>';
                    print '</tr>';
                  //  var_dump($totalpropal);die();
                    $reliquat=$totalpropal-$total;

                    print '<tr class="liste_titre">';
                    print '<td class="liste_titre"> RELIQUAT </td>';
                    print '<td> </td>';
                    print '<td> </td>';
                    print '<td align="right" class="liste_titre">'.number_format($reliquat, 0, ',', ' ').'</td>';
                    print '<td> </td>';
                print '</tr>';
