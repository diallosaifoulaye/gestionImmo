...
if (empty($reshook))
{
    ...
    && ($action == 'add' || $action == 'update') && $user->rights->societe->creer)
    {
        ...
        if (! $error)
        {
            ...
            $object->civility_id		= GETPOST('civility_id', 'alpha');	// Note: civility id is a code, not an int
          ...
          //  $object->civility =$object->civility_id;
           // $object->lastname=$object->name;
	        // Fill array 'array_options' with data from add form

	        $ret = $extrafields->setOptionalsFromPost($extralabels, $object);
            $object->poste=$object->array_options['options_profession'];
            $object->civility	=$object->civility_id;	// Note: civility id is a code, not an int
            //var_dump($object->civility_id);die;
			if ($ret < 0)
			{
				 $error++;
			}
...
else
{
    ...
    if ($action == 'create')
    {
	    ...
	    print '<td colspan="0"><input type="text" name="email" id="email" value="'.$object->email.'"   ></td>';
        print '<td>'.$form->editfieldkey('Web', 'url', '', $object, 0).'</td>';
	    print '<td colspan="0"><input type="text" name="url" id="url" value="'.$object->url.'"></td></tr>';
     ...
     /*   $i=0; $j=0;
        while ($i <= 6)
        {
            ...
            if ($idprof!='-')
            {
                ...
                print '<td>'.$form->editfieldkey($idprof, $key, '', $object, 0, 'string', '', (empty($conf->global->$idprof_mandatory)?0:0)).'</td><td>';
       ...
       /* print '<tr><td>'.$form->editfieldkey('VATIsUsed', 'assujtva_value', '', $object, 0).'</td>';
        ...
        // Civility
        print ' <tr class="individualline"><td>'.$langs->trans("UserTitle").'</td>';
          print'<td class="maxwidthonsmartphone">'.$formcompany->select_civility($contact->civilite_id).'</td></tr>';
    ...
    else
    {
       ...
       // print '<tr><td class="titlefield">'.$langs->trans('JuridicalStatus').'</td><td>'.$object->forme_juridique.'</td></tr>';
		...
		// Other attributes
		$parameters=array('socid'=>$socid, 'colspan' => ' colspan="0"', 'colspanvalue' => '0');
      //  $parameters=array('socid' =>$socid, 'colspan' =>' colspan="'.(0+(($showphoto||$showbarcode)?1:0)).'"');
